...
export function checkPhaseQueueDrained(bundlePath, { phase = 'wave' } = {}) {
  ...
  if (inFlight.length > 0) {
    finding = queueFinding(bundlePath, {
      ...
      writeTo: `node DPT_FRAMEWORK/cli/operate-work-unit.mjs inspect ${resolve(bundlePath)} --work-id ${inFlight[0].work_id}`,
    ...
    finding = queueFinding(bundlePath, {
      ...
      writeTo: delegated
        ? `node DPT_FRAMEWORK/cli/operate-work-unit.mjs claim ${resolve(bundlePath)} --phase ${phase}`
        : mainAgentOwned
          ? `node DPT_FRAMEWORK/cli/operate-queue.mjs claim ${resolve(bundlePath)}`
