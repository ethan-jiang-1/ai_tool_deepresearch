...
function submitRerun(bundleDir, record, resultPath, operation) {
  ...
  return `node DPT_FRAMEWORK/cli/operate-work-unit.mjs ${command} ${JSON.stringify(path.resolve(bundleDir))} --work-id ${JSON.stringify(record.work_id)} --result ${JSON.stringify(path.resolve(resultPath || path.join(bundleDir, record.paths.result_ref)))}`;
}
