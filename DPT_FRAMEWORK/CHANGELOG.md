...
- **breaking (delegated work-unit pipeline):** delegated sub-agent execution now uses one production path: queue demand item -> work unit -> sub-agent -> submit -> ledger -> gate. `operate-work-unit claim/submit/fail/timeout/abandon/inspect` replaces relay-slot production completion, while `operate-queue complete` remains for non-delegated queue work only.
