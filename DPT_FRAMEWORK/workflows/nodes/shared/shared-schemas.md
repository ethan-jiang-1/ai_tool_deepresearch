...
- **assignment binding**：current index / manifest / beacon carry Engine-owned read-only `assignment_contract_version: work-unit.assignment.v1`; manifest/beacon `output_contract.required_outputs[]` carries exact `path` / canonical `role` / `direct_contract`. Wave1 queue `payload.assignment_mode` is the closed `primary|supplementary` intent fact. These assignment fields are not actor-fillable and do not appear as result contract selectors.
