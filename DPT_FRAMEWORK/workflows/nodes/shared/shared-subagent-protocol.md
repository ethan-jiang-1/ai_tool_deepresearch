...
- For `suspect_transaction`, do not wait, force-timeout, delete a lock, or invent cleanup. Run `node DPT_FRAMEWORK/cli/operate-work-unit.mjs recover-transaction <bundle> --tx-id <id>` only when `repair_kind: recover_transaction` and exact `write_to` select one unlocked proof-complete v2 `started`/`suspect` journal. Resolve `<bundle>` to the projection's canonical absolute `bundle_dir`. After a changed or idempotent settled result, rerun the preserved original checkpoint. A valid global holder remains `busy`; any legacy, incomplete, unsafe, drifted, or held-suspect proof returns `missing_contract`.
...
- Run `node DPT_FRAMEWORK/cli/operate-work-unit.mjs supersede <bundle> --work-id <submitted_work_id> --reason <audit-reason>` only when the Engine selects eligible attributable post-submit drift, using the projection's canonical absolute `bundle_dir` for `<bundle>`. Read the returned predecessor work/queue IDs, committing transaction ID, `successor_queue_item_id`, and ordinary successor location. A queued successor returns to current exact-role actor observation and normal claim; an in-flight successor is polled by its existing work ID; a terminal successor follows only its own ordinary contract. Only the unique current lineage leaf's normal submit or audited late-submit restores current coverage; then rerun the same inspect/Gate.
