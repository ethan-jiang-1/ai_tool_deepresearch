// check-req-ids.mjs — Requirement ID 一致性检查
// Usage: node check-req-ids.mjs [projectRoot]
//
// Requirement ID 只有一个硬约束: 全局唯一, 只增不删, 永不复用 (req-registry.yaml)。
// 一个 id 有三种合法存在状态, 三态都不是则为 "孤儿" (疑似丢失/漏 sync):
//   alive   — 出现在 openspec/specs/ (生产 spec, 当前要构建的)
//   pending — 出现在 openspec/changes/<active>/specs/ (未归档 change 的 delta)
//   retired — 在 req-registry.yaml 该行标记 [DEPRECATED] (合法废弃, id 占位永不复用)
//
// 四项检查:
//   1. duplicate      — 同一 id 落在 ≥2 个不同的生产 spec 文件 (归属冲突; 同文件内多次引用不算)
...
// 分别收集生产 spec 和未归档 change delta 的 id + 来源文件
...
if (existsSync(changesDir)) {
  for (const entry of readdirSync(changesDir, { withFileTypes: true })) {
    ...
    for (let i = before; i < deltaOccurrences.length; i++) activeDeltaIds.add(deltaOccurrences[i].id);
  }
}
...
// 1. duplicate: 同一 id 落在 ≥2 个不同生产 spec 文件 (同文件内多次引用不算)
...
if (orphans.length > 0) {
  ...
  console.error('  合法废弃 → 在 req-registry.yaml 该行加 [DEPRECATED];疑似丢失 → 从 archive 找回或重建正文进生产 spec。');
  failed = true;
}
...
console.log(
  `All requirement IDs consistent: ${registered.size} registered` +
  ` (${retired.size} retired, ${orphans.length} orphan), ${specOccurrences.length + deltaOccurrences.length} occurrences in specs/delta.`,
);
