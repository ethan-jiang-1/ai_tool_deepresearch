...
- Shared seed-topic authoring owns the readable initialization/appendix/direction shape, and shared return-map authoring owns Wave entry shape; deterministic renderer and existing Wave inspect authority are unchanged.
- Sanctioned rerun add/update/direction-only candidates publish through existing atomic topic-state apply/recover, while rerun-ready checks only plan-bound direction structure and profile-count synchronization—not direction semantics or Agent judgment.
