...
Three subagents, SAME template (`dpt-source-intake`), THREE different companies.
Proves that each subagent instance has a unique identity:
different `agentId`, different search path, different sources found.

Bundle: `dpt_rb_test_gs_identity/`, trace: `_trace_gs_identity.jsonl`.
