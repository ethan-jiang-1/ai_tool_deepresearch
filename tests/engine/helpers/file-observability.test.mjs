...
// Tests for auditFileObservability — validates 6 classifications and relay slot handling
// @impl 8A.13, 8A.16
...
describe('file observability', () => {
  ...
  it('classifies ledger-declared files as declared_authoritative', () => {
    ...
    writeFileSync(join(dir, 'rb_output_declarations.jsonl'), JSON.stringify({
      ...
      output_files: [{ path: 'reference/topic-a-source1.md', role: 'reference', source_url: 'https://example.com/news/a' }],
      cache_trails: [],
    ...
    const result = auditFileObservability(dir, {
      ...
      ledgerDeclarations: [{
        declared_at: new Date().toISOString(),
        work_id: 'w1',
        output_files: [{ path: 'reference/topic-a-source1.md', role: 'reference' }],
      }],
    });
  ...
  it('classifies relay slot runtime-receipt.jsonl as expected when at correct path depth', () => {
    const dir = setupBundle('fo-relay', {
