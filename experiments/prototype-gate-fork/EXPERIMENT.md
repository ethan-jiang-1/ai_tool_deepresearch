## 集成测试结果 — 24 tests, 5 suites, 0 failures
...
4. **与 gate-loop 的差异**：`convergeRepair` 在 maxIterations 耗尽后返回具体 branch 名（如 `"fail_a"`），而非 gate-loop 的 `"escalated"` 哨兵值。这个选择保持了 fork 路由的语义透明——调用方可以看到"卡在哪个分支上"。

5. **命名对齐**：两个实验使用一致的 API 命名约定 (evaluate/evaluateBranch, router/forkRouter, repairLoop/convergeRepair, segmentRegistry/loadNextSegment)，便于将来抽象到共用库。
