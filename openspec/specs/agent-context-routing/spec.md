...
copying it in full. Its compressed definitions SHALL preserve that an active
bundle may be the selected production-run or disposable-experiment runtime
context, Gate verdict feedback does not itself select the next Chain phase, and
a Source of Record does not by itself grant authority, capability, permission,
liveness, or evidence.

`CONTEXT.md` SHALL link
`docs/adr/0001-keep-agent-flow-markdown-driven-and-engine-gated.md` as an
optional architecture rationale. That link SHALL not make the ADR a mandatory
pre-task read, a behavior authority, or a replacement for the Charter.
...
behavior authority. Their shared-project pre-read blocks SHALL be identically
...
the framework README SHALL state the same boundary in its parent-project route.
...
- **AND** the local routes SHALL preserve the framework README, COMMANDS, and
  applicable playbooks as the framework operating surfaces
- **AND** no framework-local context glossary SHALL be introduced

#### Scenario: Context routing does not replace selected DPT entry routing
...
that reads the actual root and framework `AGENTS.md` / `CLAUDE.md` behavior
files and `README.md` entry documents. It SHALL verify the required
...
Else` blocks and before the existing root Deep-Research-Routing,
root directory-routing, framework first-priority routing, framework
`> **最快触发**` callout, and framework trigger block. It SHALL also verify
the paired behavior-file pre-read blocks stay synchronized; that root/framework
...
`tests/integration/md/dpt-research-entry-routing-contract.test.mjs` alongside
the focused regression to establish that the context pre-read preserved that
contract.
...
- **WHEN** any root or framework Agent behavior file or README entry document
  omits the required glossary route, places `CONTEXT.md` before the Charter,
  the root drops its normal instruction-discovery/task-specific-authority or
  scoped-reading boundary, moves its Context route outside `## Before Anything
  Else`, or lets the pre-read fall after an existing root routing block,
  framework first-priority routing, `最快触发` callout, or framework trigger
  block, either behavior-file pair drifts in its required
  synchronized pre-read block, the framework drops its README, COMMANDS, or
  selected-playbook route after the pre-read, root entry documents drop the
