...
#### Scenario: pre-TopicTreeEvolution canonical plan remains readable
- **WHEN** a valid canonical entry has no `previous_layouts` field
- **THEN** canonical parsing SHALL treat it as empty layout history

#### Scenario: Pre-TopicTreeEvolution coordinate shape remains readable
- **WHEN** a pre-TopicTreeEvolution canonical plan uses an accepted non-normalized current id/slug shape
