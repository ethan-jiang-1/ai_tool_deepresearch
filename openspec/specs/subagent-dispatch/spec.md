...
Current dispatch guidance SHALL define dispatch as work-unit claim and prompt handoff. It SHALL NOT describe gate pass as declaring retired dispatch surfaces, tasks, or result schemas as production dispatch authority.
...
- **AND** it SHALL NOT describe bounded retired slots as the production dispatch model
...
Batched fan-out SHALL be expressed as multiple Engine-allocated work units, not as a slot array or retired dispatch manifest.
...
- **AND** its verdict SHALL not depend on retired dispatch manifests
...
The cap SHALL be described in work-unit terms. Current production guidance SHALL NOT express the concurrency cap as retired slot count.
