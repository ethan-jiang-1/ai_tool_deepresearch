...
- **THEN** `gate` 字段的值 MUST 为 `wave0_complete`

### Requirement: Gate CLI skeleton shape

每个 `check-gate-<name>.mjs` SHALL：

1. 使用 `node:util.parseArgs` 解析 `--bundle` 参数
2. `--bundle` 未提供时 SHALL `console.error` 并 `process.exit(2)`
3. 输出 SHALL 为合法 JSON，包含以下 shape：

```json
{
  "check": { "passed": true, "gate": "<gate-name>" },
  "inspect": [],
  "advice": []
}
```
...
- **WHEN** `node check-gate-wave0-complete.mjs --bundle dpt_rb_test` 被调用
- **THEN** stdout MUST 是合法 JSON，MUST 包含 `check`（含 `passed` 和 `gate`）、`inspect`、`advice` 三个 key
...
- **WHEN** agent 需要运行 `wave0_complete` gate
- **THEN** agent MUST 调用 `node DPT_FRAMEWORK/cli/gates/check-gate-wave0-complete.mjs --bundle <path>`，MUST NOT 调用一个 generic runner 加 `--gate wave0_complete` 参数
