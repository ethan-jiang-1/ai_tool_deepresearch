...
The existing `other -> canonical role` normalization SHALL remain only for a legacy attempt whose index record genuinely lacks `assignment_contract_version` because it was claimed before this contract existed. An unclaimed queue card never receives this compatibility and must first satisfy current assignment-mode validation. Legacy selection SHALL use the claimed attempt's marker absence and hash-bound manifest/index facts; it SHALL NOT infer compatibility from current framework version, bundle creation version, queue ID, path shape alone, or caller choice. The legacy submit transaction SHALL continue to emit a visible normalization diagnostic naming output path, submitted role, normalized canonical role, and reason.
