...
- 将 HITL1 probe 保持为至多一次 neutral search 和第一个实际 HTTP(S) 搜索结果，但允许 native surface 不存在或 native fetch 未返回真实 page content（blocked / unavailable / failed）后，对同一 URL 再执行至多一个 shell fallback：`curl --fail --silent --show-error --location --max-time 15 --proto '=http,https' --proto-redir '=http,https' -- "<same-url>"`。
...
- 更新 focused Markdown contract tests 和现有 case-115 真实 HITL1 Subject Agent probe，使 deterministic evidence 只证明 bounded guidance/shape；从同一次 retained Subject transcript 能观察到 native blocked、`curl` 成功分支时才证明 fallback，未观察到时仅将该分支 claim 诚实记录为 `NOT_RUN`，不复制第二个 canary。
...
- Apply 预计修改 `DPT_FRAMEWORK/workflows/nodes/phases/phase-hitl1.md`、focused `tests/integration/md/phase-hitl1-research-access.test.mjs`、现有 case-115 Subject runner/playbook及其 focused contract assertions；不增加 Subject identity 或 playbook manifest entry。
