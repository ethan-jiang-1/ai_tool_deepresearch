...
- 在 enrichment apply 的决策点复用现有 `evaluateSeedTopicAuthoring()`，对写入前可解析的 canonical drift 返回被同一 writer 修复的一个最早 binding root，并对写入后的 canonical envelope 做同源断言；strict input/schema failure 在写入前失败并指向同一个 apply checkpoint。Queue completion、topic-state inspect 和最终 seed-topics Gate 继续复用该 evaluator，不新增 lint CLI、hash identity 或第二套 schema。
- 更新 `phase-seed-topics.md`、shared seed authoring contract、task-card action/done condition 和 topic-state command guidance：Agent 通过 structured enrichment input 写 frontmatter，只直接编辑非权威 Markdown body；`must_answer` 由 Engine 从 registry 复制，绝不由 Agent 摘要或重写。
- 调整 `seed_topic_materialize` completion 的 fallback feedback：canonical binding drift 指向 topic-state enrichment apply 这个合法 Engine operation，正文或 input 修复后仍回到原 enrichment/queue completion checkpoint；不再把 raw canonical YAML edit 描述成正常修复路径。
...
- 测试面：topic-state helper/CLI 的 exact parsed-value round trip、strict enrichment rejection、body preservation与crash recovery；queue/gate shared-evaluator feedback；Markdown wiring与duplicate-authoring static regression；一个真实 Seed Topics Agent-flow canary验证正常路径不再手写 canonical YAML。
