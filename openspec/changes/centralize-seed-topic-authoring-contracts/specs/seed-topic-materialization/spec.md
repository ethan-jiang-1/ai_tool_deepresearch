...
`phase-seed-topics.md`, rerun add-topic guidance, and Wave phase guidance SHALL reference the loaded shared contract at their decision points and SHALL NOT each carry an independently maintained complete seed skeleton. The deterministic new-seed renderer SHALL emit the same ordered appendix headings and token set as the shared canonical skeleton. Executable parity tests SHALL fail on a missing, extra, reordered, or renamed canonical appendix heading/token while ignoring prose bytes, YAML mapping order, and presentation-only whitespace.
