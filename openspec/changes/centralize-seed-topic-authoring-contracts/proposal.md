...
Net simplification 来自以一个 shared readable surface 替换 phase 间重复的完整 skeleton/direction 示例、删除 Wave consumer 的本地 action regex，并把 direction publication并入现有 topic-state transaction；本 change 明确避免 template registry、通用 linter、第二 renderer、额外 affected-topic state 和新的修复控制层。用户只拥有 rerun 语义与 rationale 决定，Agent负责形成 candidate并执行机械修复，Engine只裁决可解析结构、round/action binding、required-field presence与existing transaction；`human-directed` 不创造写入权限或绕过能力。
...
- `canonical-topic-state`: sanctioned rerun add/update action携带结构化 direction candidate，并由现有 crash-safe plan+seed workspace原子发布，不新增 mutation path。
- `research-return-map`: 让各 Wave/role producer加载同一个canonical entry/section responsibility contract；generated work-unit cue以静态field parity保持自包含，且不改变 `RRM-007` authority/projection verdict。
...
- 主要实现面：`DPT_FRAMEWORK/workflows/nodes/shared/`、`phase-seed-topics.md`、`phase-rerun.md`、`phase-wave0.md`、`phase-wave1.md`、`phase-wave2.md`、workflow package/template validation、`canonical-topic-state.mjs` 的 apply input/seed renderer/workspace，以及现有 rerun direction parser/check owner。
- 测试面：shared-context reachability、renderer parity、legacy direction compatibility、required-field/root-first diagnostics、rerun-ready integration 与必要的 deterministic rerun chain。
