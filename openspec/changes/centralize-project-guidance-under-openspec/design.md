...
- 让 accepted specs、supported operation adapters 和 focused regressions 与新路径在同一 Apply 收敛。
...
### Root adapters and no compatibility root
...
All known repository consumers change in the same Apply. The old root is removed only after its consumers and
internal links resolve. A current external consumer discovered during Apply is a blocker until an explicit,
bounded compatibility decision is approved; guessing one into existence violates the one-canonical-path goal.
...
path per document and every selected consumer uses it. Recovery means repair within the selected change and
rerun focused checks, not copying documents back to create a second source.
...
supported operation guidance, current/live old-path absence, and preservation of selected research-entry routing.
