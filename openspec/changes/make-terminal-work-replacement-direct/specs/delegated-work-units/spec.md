...
`replace` SHALL allocate no work ID, envelope, receipt, result, cache trail, source claim, ledger row, Gate result, or queue completion. Its successful result SHALL name the replacement `queue_item_id`, its queue location, the parent terminal fact, and the existing role-bound `claim` checkpoint. A current actor observation remains required before claim can allocate a fresh work ID.
...
#### Scenario: replacement is idempotent only while the same successor is live

- **WHEN** the exact replacement demand for a terminal parent is already queued or delegated in flight
- **THEN** another `replace` call SHALL return that same queue-item identity without creating another demand
- **AND** when that successor has terminal history, the earlier parent SHALL not create another successor
