...
本 change **不修 BUG-019 对策**，而是先把观测与取证做出来：让 sub-agent logging 真正在 runtime 活过来，埋下抗手糊的信号（beacon 模式 + UUID nonce + nonce-anchored engine trace 链）。等真实 run 之后，下一个 coding agent 据此判决 BUG-019（plan §10 判断手册；本 change 经 RPG-010 把它嵌入为框架自带指南）。**取证本轮 diagnostic-only（不改 pass/fail）；execution-based 阻断升级 deferred 到未来 change。**

> **抗手糊是频谱，非二元（第 4 轮修正）**：早先"Layer-1 nonce 抓伪造"的框定**反了**——`dispatch.json`/`_beacon.json`/`_agent.json`/receipt 都是文件，可被 `writeFileSync` 直接伪造，nonce 只是 UUID 形状非引擎签名；S1/S2 只抓**懒手糊**（BUG-019）。真·难伪造的是 engine 经 `traceEntry` 写进 `rb_trace.jsonl` 的**事件链**。故本 change (i) 用 **SUD-007** 把 nonce 贯穿 staging+commit trace 事件（链可交叉校验），(ii) 加 **RPG-012** 跨文件一致性诊断，(iii) ship 的指南（RPG-010）明示频谱、不夸大。无密码学签名（out-of-scope）。
...
- **Engine**：`DPT_FRAMEWORK/engine/subagent-relay.mjs`（`createSlot`/`createDispatchManifest`/`buildSpawnPrompt`/`taskMarkdownForSlot`：写 beacon + 持久化 nonce + spawn prompt 只传 slot 目录；**SUD-007：4 个 `traceEntry` 站点 [L586/606/928/1015] 加 `receiptNonce`**）；gate helpers（forensic 诊断含 RPG-012 跨文件一致性检查，advisory only）。
