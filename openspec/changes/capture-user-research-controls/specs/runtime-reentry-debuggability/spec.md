...
`enter-phase` SHALL reject a setup-ready handoff whose route-bound checkpoint is missing, whose checkpoint path, `gate_attempt_id`, or plan hash disagrees with the route trace, or whose bound plan hash no longer agrees with the current plan bytes. It SHALL not treat a later append-only failure trace as a retraction of a prior passed route. This requirement adds neither a second checkpoint authority nor a drift exemption.
...
When multiple checkpoint manifests exist, reentry tooling SHALL select the latest checkpoint whose `gate_result_ref.gate` or `normalized_target.status_gate` matches the requested target. A route-pending setup-ready checkpoint without its bound trace SHALL be inspectable recovery evidence but SHALL NOT qualify as a passed handoff checkpoint. If no matching checkpoint exists, tooling MAY fall back to the latest checkpoint for global drift context, but SHALL report the absence of a target-matching checkpoint as inspect/advice.
...
- **THEN** `check-reentry --at setup_ready` reports the missing passed checkpoint baseline
