...
- [x] 10.5 Add/run required heavy Sub-agent actor playbook for BUG-039 and BUG-040 when claiming the producer path is fixed: a native Sub-agent receives a work-unit task, reads the beacon/task/schema, writes required result/receipt/output/cache files before returning, preserves exact nonce/identity fields, and submit succeeds without Phase Agent transcribing chat text.
- [x] 10.6 Add/run heavy Sub-agent or standard controller playbook for BUG-037 if the apply claim includes real producer containment: the work unit completes with all writes under `bundle_dir`, repo root remains free of runtime leaks, and inspection feedback is visible to the Phase Agent.
...
- [x] 12.6 Slice F controlled playbook evidence: run required task-10 playbooks after the relevant deterministic guardrails are implemented, and treat failed playbooks as design feedback rather than as flaky optional demos.
...
- [x] 13.2 Run required controlled playbooks from task 10 and record which bug-probes they close versus which deterministic guardrails they merely complement.
...
- Heavy native Sub-agent playbooks run with real native Sub-agent actors and recorded PASS in `_temp/exp_verdicts.jsonl`: case-604 closes the BUG-039/040 actor-boundary claim for write-before-return, exact nonce preservation, and successful submit without Phase Agent transcription; case-605 closes the BUG-037 real producer containment claim for bundle-root writes, clean repo-root leak scan, and visible inspect feedback.
- Slice F treated failed playbook evidence as design feedback: the first case-605 attempt used real Sub-agent writes but declared `result.cache_trails` as leaf file paths (`websearch.json`/`page.md`/`meta.json`), and submit correctly rejected it with `cache_trails path is not a directory`. The contract was clarified in generated task/spawn text, playbook instructions, and focused regression assertions so `cache_trails` names bundle-relative cache leaf directories only; the retry passed.
- Residual risk after heavy canaries: actor-boundary evidence is real but still controlled/disposable, not a real-environment web run. The remaining risk is ordinary production variance in future Sub-agent compliance; deterministic submit/cache/nonce/bundle containment guardrails fail closed when the actor violates the contract.
