...
- [ ] 2.1 Implement AGQ-022 in `shared-subagent-protocol.md`: document bounded top-up batch claim calculation, explicit `operate-work-unit claim --count <claim-count>`, per-work-id fan-out, reconstructed in-flight tracking, accepted/default cap, remaining free capacity, and legal serial exceptions.
...
- [ ] 3.5 Update any phase-local command examples for AGQ-022 and SWE-006 so examples use explicit `--count <claim-count>`, `inspect` or filesystem polling, `submit`, and terminal commands rather than passive waiting.
...
- [ ] 5.5 Implement RWG-017 in Wave2 gate definitions/CLIs: accept existing-backed pure-synthesis `00-cross` references with `W2F-xxx` and prior-wave backing refs, require accepted consumer-facing backed findings to have cross references or explicit omission reasons, reject synthetic or newly searched `source_url` values without targeted evidence, and reject new evidence claims missing targeted work-unit coverage.
...
- [ ] 6.3 Add gate/provenance tests for WTS-010, WPG-005, WPG-012, and RWG-017: Wave2 accepts an existing-backed `reference/00-cross-*.md` with `W2F-xxx`, finding index or ledger refs, locator refs that resolve to submitted backing, and concrete prior Wave0/Wave1 source/cache/degraded-capture/work-unit backing.
