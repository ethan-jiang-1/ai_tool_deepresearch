...
- Wave0/Wave1 phase guidance 改为对独立 delegated demand 默认使用 bounded batched claim：Phase Agent 计算并发上限后显式调用 `operate-work-unit claim --count <cap>`，不再把 `--count 1` 当正常策略。CLI 默认值不在本 change 中悄悄改变。
- Shared delegated protocol 和 silent execution guidance 增加 active polling loop：Phase Agent spawn background Sub-agents 后主动检查 work-unit result/receipt/output readiness，ready 就 submit，rejection 就 repair 或 terminalize，不等待用户 "continue"、task-notification、unrelated background state。
- Wave1 reference materialization 改为 Phase-owned post-submit projection：Sub-agent 负责搜索/fetch、`evidence-summary.md`、`question-list.md`、`source_claims[]`、cache trails、result/receipt；Phase Agent 在 successful submit 后从已提交 source claims/cache/ledger rows 写 `reference/{topic_slug}-<source-slug>.md` 和更新 `reference/_INDEX.md`。
- Wave2 pure synthesis path 显式物化 `reference/00-cross-*.md`，但只允许基于已有已提交 Wave0/Wave1 evidence 的 concrete backing。新外部 search/evidence 仍必须通过 `wave2_targeted_evidence` work-unit submit 才能作为 fetched-source delegated evidence。
...
- `reference-flat-format`: Reference format SHALL support Phase-owned materialized references as consumer projections backed by submitted source claims/cache/ledger rows, without treating them as alternate delegated authority.
...
- `agentic-queue`: Wave0/Wave1 queue-loop guidance SHALL prefer bounded batched fan-out for independent eligible delegated demand while keeping Engine allocation authority in `operate-work-unit claim`.
