...
The current repair-loop role SHALL keep gate failure, Engine inspect/advice feedback, Agent-visible repair, rerun of the same gate, and before/after runtime evidence visible in Markdown. Its V2 `verdict_mode` SHALL be `last`; the finalizer SHALL apply that policy rather than an outer runner re-evaluating every historical failed check.

#### Scenario: Repair loop preserves final-per-gate semantics

- **WHEN** a gate fails, the Playbook Agent consumes feedback, repairs the active bundle, and the same gate passes
