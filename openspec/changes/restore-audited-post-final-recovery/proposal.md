...
- 新增一个窄的 post-final recovery operation family，复用现有 handoff、topic-state 与 reentry owners，只提供 `inspect|apply|recover`：
  - `inspect` 从 terminal Final lineage、status/current node、pending workspace、canonical topic state 与现有 integrity facts生成一个 eligibility verdict和至多一个最近动作；
...
- 每次 apply/recover 复用现有 direct validators，在发布 reentry handoff 前短路检查 terminal lineage、rerun limit、quiescence、C2/C3 workspace、canonical footprint与目标 status/profile shape；失败只留下 accepted recoverable workspace或保持原状态，并返回一个 exact recover/repair action。完成event→enter→advance后使用现有 source-window checkpoint `check-reentry --at hitl2_recorded`，并由recovery summary/current_node/event+load+phase_transition证明rerun entry；不误用要求phase自身已pass的`--at phase-rerun`，也不新增第二套 recovery controller。
