...
The canonical Agent-facing return-map entry example, Wave-to-section ownership, and one-time token lifecycle are owned by the seed-topic document-shape template (`workflows/nodes/templates/seed-topic-template.md`) together with the canonical topic-state operation playbook (`command_playbook/operate-topic-state.md`). The `shared-return-map-authoring` shared node SHALL remain a compatibility pointer that loads through phase `requires` chains and intentionally defines no second entry grammar, slot ownership, token lifecycle, or writer path. Authoring guidance SHALL state that evidence collection, extraction, synthesis, and seed-topic backfill all carry short meaning statements plus refs so later Agents can navigate the map without rereading the whole bundle blindly. Return-map entries SHALL expose the same minimum entry-local fields for Wave0, Wave1, and Wave2: `evidence_meaning`, `relationship`, `refs`, `status`, and `next_hop`, plus optional entry-local identity/metadata defined by the accepted return-map contract. Authoring guidance SHALL state that refs are bundle-relative, concrete consumer navigation comes first when materialized, and internal artifact/cache/work-unit refs remain secondary provenance.
...
- **THEN** the loaded guidance chain SHALL resolve the five common entry fields and wave-to-section mapping through the canonical template and playbook owners
...
- **AND** static parity SHALL fail if its canonical field set or authority disclaimer drifts from the canonical owners
...
- **THEN** it SHALL find one complete generic return-map template in the canonical template surface (`templates/seed-topic-template.md`)
...
- **THEN** the canonical authoring guidance SHALL describe the token as consumed
...
- **AND** the canonical authoring guidance SHALL direct repair to the named seed coordinate and rerun that same inspect
...
The per-wave backfill token filter SHALL accept the current target wave and match only tokens belonging to that wave:
...
When the per-wave token filter matches a token belonging to the target wave, return-map validation for that wave SHALL be skipped. Tokens belonging to other waves SHALL NOT cause a skip.

Each Wave return-map inspection SHALL pass its current target wave to that filter. The `__BACKFILL_PENDING_QUESTIONS__` token is assigned to Wave1 (Wave1 is its primary consumer; Wave2 appends W2F-xxx entries when no token is present).
...
- **AND** Wave0 inspect runs with target wave `wave0`
- **THEN** the per-wave token filter SHALL NOT match for that target (the present token belongs to Wave1)
...
- **AND** Wave2 inspect runs with target wave `wave2`
- **THEN** the per-wave token filter SHALL NOT match for that target (token belongs to Wave1)
...
reparse seed Markdown. The formerly separate Wave1 token definition rules
(`no_stale_mechanisms_token`, `no_stale_trends_token`, and
`no_stale_pending_questions_token`) and the formerly separate Wave2 token
definition rules (`backfill_judgment_token_absent` and
`backfill_questions_token_absent`) are retired historical names: no rule under
those names exists or runs. They were duplicate token
validators, not independent authority checks; their retirement does not make token
