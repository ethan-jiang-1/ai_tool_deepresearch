...
#### Scenario: Keyed profiles keep the existing strict comparison

- **WHEN** both the current profile and the event's committed-after semantics carry `human_decision_checkpoints.hitl2.rerun_count`
- **THEN** the stage evaluator SHALL apply the existing exact count-window and full profile comparison without any absence tolerance
