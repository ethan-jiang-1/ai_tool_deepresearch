# silent-wave-execution Delta Spec
...
每个 manifest lifecycle `stop: no` phase SHALL 通过其 frontmatter `requires` 字段加载 `shared/shared-silent-execution.md`，这是 **WNC-009** 的强制性要求。该文件定义了静默执行的完整行为契约，包括绝对禁令（§0）、降级优先级链（§1）、状态规则（§2）、冲突覆盖（§3）、用户消息处理（§4）和疲劳抵抗（§5）。Relay/sub-agent task surfaces（例如 `phase-wave2-subagent.md`）不属于本 requirement 的 lifecycle phase 范围。Agent SHALL 在进入 lifecycle phase body 之前阅读该契约。
