...
- 为 disposable 与 production bundle creator 建立共同的外部行为：在任何 `mkdir`、模板/控制文件写入、trace/log 写入或 bundle validation 前，先解析完整 argv 并决定 `--` delimiter 之前的 standalone `--help` option、缺参、未知或重复 option、缺少 option 值及名称是否合法；任何由 `--target-dir` 派生的 bundle path 都必须以子进程 argument vector 传递，不能插入 shell command string。
- `--` delimiter 之前的 standalone `--help` option 必须优先打印 usage、以 0 退出且没有文件系统副作用；delimiter 后的同一文本仍是 positional，不能成为合法 name。
- 两个 creator 只接受其各自已声明的 option 与合法名称 suffix。非法名称（flag、路径分隔/遍历、空白、错误首字符或不符合对应 production/disposable 命名字符集）必须在 target directory 创建前失败，并给出一个最近的可执行修复动作；disposable `--case` 保持实际 experiment identity 所需的 `case-<digits>` 形状。
...
- **BREAKING（仅对无效调用）**：此前被误当作名称的 `--help`、未知/重复 flag、非法名称、非 canonical `--case` 值或缺少 option 值不再创建 bundle，而是立即失败或显示帮助。
...
同一 `experiment-shared-infra` delta 也会校正 EXS-002 的历史描述：该 creator 实际上以随机 hex suffix 创建 disposable bundle，主规格中省略 suffix 的示例不能与 implementation、Gate 或 EXS-003 同时为真。
...
Direct Source of Record 是传入 argv、显式 target directory 与 creator 自己构造的最终 basename；最短合法闭环为“解析/验证一次 → 有效时创建一次，或无效时零写入并返回 usage/错误”。用户只提供或授权调用输入；Agent 负责修正可机械判定的命令形状；Engine/CLI 仅裁决结构合法性，不推断名称意图或取得额外 mutation authority。`--target-dir` 是路径而非 bundle identity，不以脆弱的命名过滤限制它，而是通过 argument-vector 调用保留其字面路径安全性。

净简化是把两个散落的裸位置解析收敛为各 creator 的单一早期边界，并让测试直接断言零副作用；它避免继续依赖后置 gate、历史人工清理或每个调用点记住“不要把 flag 放第一个”的隐含规则。它不为 `--nodes` 新建 source sandbox：该 option 是调用者选择的 experiment input，当前没有跨权限边界需要由此 creator 判定。
