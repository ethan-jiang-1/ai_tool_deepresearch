...
For Wave1 topic deepening, required delegated output coverage SHALL include submitted `evidence-summary.md`, `question-list.md`, topic reference files, and verified cache trails. A depth-review projection is Phase Agent review output, not delegated work-unit output authority. Gates MAY validate `depth-review.yaml` as a phase-owned consistency projection, but work-unit provenance checks SHALL only use it to cross-check that every reviewed source/reference/cache ref binds back to submitted work-unit ledger rows.

A depth-review projection MAY be written by the Phase Agent after submit, but it SHALL name the submitted work-unit rows and source/cache refs it reviewed; filesystem-only Wave1 outputs SHALL NOT become coverage authority.
...
#### Scenario: Wave1 depth review cannot create delegated coverage
