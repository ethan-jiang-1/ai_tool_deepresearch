> req: CDP-005, CDP-006, CDP-007, CDP-008
...
proven later append. Final has no Gate CLI and SHALL NOT emit a trace event that
claims a stop-authorization transition — the reserved `final_delivery`
stop-authorization value is a queue stop state, not a trace event name — or a
satisfaction event.
...
stop-authorization or satisfaction trace event (including any event named after
the reserved `final_delivery` stop-authorization value), no hidden next edge, and no
