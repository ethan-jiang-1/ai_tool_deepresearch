...
- Reason: 粒度拆分(spec-lean mainline C3 megablock-requirement-split):Post-final recovery SHALL expose one direct eligibility and request contract 超粒度,按深挖定稿分界拆为单主题子块。
- Migration: requirement 文本逐字节守恒拆为 6 个子 requirement: Post-final recovery CLI SHALL expose one direct eligibility inspection / Post-final apply SHALL validate the closed action and rerun budget / ReopenResearchPass workspace precedence and evaluator authority SHALL stay deterministic / Committed post-final lineage SHALL remain recognizable through rerun and handoffs / Final inventory lineage witnesses SHALL be deterministic sorted digests over safe files / Post-final apply SHALL retain strict request and verdict contracts。requirement 身份 = 标题稳定锚点;registry 与 spec header 零触碰(无新增/废弃 ID)。
