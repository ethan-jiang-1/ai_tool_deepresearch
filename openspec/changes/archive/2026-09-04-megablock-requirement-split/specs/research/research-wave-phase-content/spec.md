...
- Reason: 粒度拆分(spec-lean mainline C3 megablock-requirement-split):Wave1 phase body completeness with subagent boundary 超粒度,按深挖定稿分界拆为单主题子块。
- Migration: requirement 文本逐字节守恒拆为 3 个子 requirement: Wave1 phase body SHALL materialize closeout through narrow index repair and depth review / Wave1 dry-submit and shared template guidance SHALL precede formal submit / Wave1 focus context SHALL convert into bounded supplementary commitments。requirement 身份 = 标题稳定锚点;registry 与 spec header 零触碰(无新增/废弃 ID)。
