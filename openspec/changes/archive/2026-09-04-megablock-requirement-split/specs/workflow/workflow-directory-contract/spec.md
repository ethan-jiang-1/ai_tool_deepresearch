...
- Reason: 粒度拆分(spec-lean mainline C3 megablock-requirement-split):Runtime bundle canonical structure 超粒度,按深挖定稿分界拆为单主题子块。
- Migration: requirement 文本逐字节守恒拆为 2 个子 requirement: Runtime bundle roots and CLI path semantics SHALL distinguish coordinates / Harness-root rules and canonical runtime surfaces SHALL stay bundle-scoped。requirement 身份 = 标题稳定锚点;registry 与 spec header 零触碰(无新增/废弃 ID)。
