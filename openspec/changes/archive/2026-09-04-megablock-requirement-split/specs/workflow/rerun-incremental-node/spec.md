...
- Reason: 粒度拆分(spec-lean mainline C3 megablock-requirement-split):Rerun node analyzes rationale vs seed_topics and produces topic adjustment plan 超粒度,按深挖定稿分界拆为单主题子块。
- Migration: requirement 文本逐字节守恒拆为 2 个子 requirement: Rerun adjustment plan and apply forms (K1) / Rerun authorization and execution loop (K2)。requirement 身份 = 标题稳定锚点;registry 与 spec header 零触碰(无新增/废弃 ID)。
