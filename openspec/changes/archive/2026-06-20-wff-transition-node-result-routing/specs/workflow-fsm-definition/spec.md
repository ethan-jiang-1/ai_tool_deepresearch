...
FSM 定义文件 (.fsm.json) 的加载与校验。定义工作流的 node-keyed `states` 和 transition rules，由 Zod schema 校验结构合法性，是 workflow-fsm-definition 的 Source of Record。

## RENAMED Requirements

- FROM: `### Requirement: FSM definition file SHALL be loadable and validatable`
- TO: `### Requirement: Node-keyed FSM definition file SHALL be loadable and validatable`

## MODIFIED Requirements

### Requirement: Node-keyed FSM definition file SHALL be loadable and validatable

The system SHALL load a `.fsm.json` file that defines the workflow's states and transitions. The file SHALL contain a `name`, an `initial` state, and a `states` map where each key is a canonical node file reference and each value is an object with an `on` map keyed by the public outcome vocabulary `passed` and `failed`. The `initial` node SHALL exist in the `states` map. Transition targets SHALL be either a node file reference (string) or `null` (terminal state).
...
#### Scenario: Unexpected outcome key is rejected

- **WHEN** a state's `on` map includes an outcome key other than `passed` or `failed`
- **THEN** the system SHALL throw a validation error

#### Scenario: Null transition target marks terminal state

- **WHEN** a state's `on.<outcome>` value is `null`
