...
JavaScript Gate 是 deterministic checkpoint。Gate 检查当前 workflow state，并把 next-node feedback 交给详细 transition router 生成。Gate 不加载 Markdown node，也不拥有 Agent Flow。
