...
- Wave0/Wave1 phase guidance 改为对独立 delegated demand 默认使用 bounded top-up claim：Phase Agent 计算并发上限、eligible demand、当前 in-flight 占用后的 effective claim count，并显式调用 `operate-work-unit claim --count <claim-count>`，不再把 `--count 1` 当正常策略。CLI 默认值不在本 change 中悄悄改变。
...
- Reference classification 使用现有 deterministic bundle surfaces：reference metadata、`_INDEX.md source_layer`、submitted source claims、accepted source URL surfaces、cache/degraded-capture/work-unit ledgers、output declarations、Wave2 `W2F-xxx` ledger/index refs；本 change 不新增 required metadata key 或 `_INDEX.md` authority column，无法分类时 fail closed。
...
- `work-unit-provenance-gate`: Provenance checks SHALL distinguish delegated fetched-source output coverage from Phase-owned projection references; new fetched evidence without submitted work-unit backing fails as delegated bypass, while unclassifiable or unbacked projections fail as backing drift.
...
- 相关 requirement IDs 已登记在 `openspec/governance/req-registry.yaml`；本 change 不新增 capability prefix。
