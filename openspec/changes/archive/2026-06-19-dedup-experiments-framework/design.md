...
- 统一 trace writer 位于 `DPT_FRAMEWORK/engine/trace.mjs`，与 `schema/contracts/trace.mjs` 配对
- 实验共享基础设施位于 `experiments/shared/`（不进生产环境）
- 引擎间代码重复用 import 替代（`workflow-fsm` → `workflow-loader`）。注意：`subagent-relay` 是独立运行时（启动真实 native subagent），其 fork/repair/dispatch 逻辑与 `gate-fork` 的通用 `forkGate` 无关——subagent 不 import gate-fork
- `schema/contracts/queue.mjs` 从骨架升级
- 所有 19 个 playbook 和 7 个测试（1 个 trace + 6 个 engine）的 import 路径更新后仍然 PASS
...
| `experiments/prototype-workflow-next/workflow-next.mjs` | `DPT_FRAMEWORK/engine/workflow-chain.mjs` |
...
### Decision 2: `trace.mjs` → `DPT_FRAMEWORK/engine/trace.mjs`

**选择**：将统一后的 trace writer 放在 `DPT_FRAMEWORK/engine/trace.mjs`。
...
**统一方案**：6 份拷贝有 3 种变体（静默版、彩色 echo 版、图标版），API 完全一致。统一为 `createTrace(filePath, options?)` 工厂函数，通过 options 控制 console echo 和图标集。当前各 engine 的调用代码改为传入对应的 options。
...
**选择**：
- `workflow-fsm.mjs` 不再内嵌 loader 逻辑，改为 `import { resolveDependencyClosure, executeLoadPlan, createState, createWorkflowRuntime, nodePath, parseFrontmatter, readMarkdownFile } from './workflow-chain.mjs'`

**不适用 import 的情况**：
- `subagent-relay.mjs` 的 `evaluateBranch`/`forkRouter`/`convergeRepair` 看起来像 gate-fork 的功能，但它们是 subagent 独立运行时的一部分（启动真实 native subagent 的 dispatch→spawn→collect→merge 链路）。Phase 4 已将 `gate-fork.mjs` 重写为通用 `forkGate(state, rules, branches)`——零领域知识。subagent 保留自己完整的 fork/repair 实现，不从 gate-fork import。

**理由**：引擎搬入同一目录 `DPT_FRAMEWORK/engine/` 后，真正有代码重复的模块可以互相 import。之前各 prototype 目录互相隔离（在 `experiments/` 下），代码只能复制。现在消除这些复制，让依赖关系显式化。但不是所有功能相似都算重复——独立运行时的领域逻辑不应强行合并到通用引擎。
...
### Decision 6: `command_experiments/` 移出 `DPT_FRAMEWORK/`

**选择**：`DPT_FRAMEWORK/command_experiments/` → `experiments_playbook/`（repo 根目录），与 `experiments/` 并列。

**理由**：`command_experiments/` 是实验入口 playbook（Agent 驱动的受控 E2E），不是框架代码。`DPT_FRAMEWORK/` 应保持纯框架（engine + schema + CLI），可分发。移出后实验相关全部聚在一处：`experiments_playbook/`（入口）+ `experiments/`（prototype fixtures + shared 工具）。

Playbook MD 内的 bash heredoc JS import 路径相对于 disposable bundle（`$B/`，repo 根），不依赖 playbook 文件自身位置 → 移动目录不需要改任何 JS import 路径。

同步清理：6 个 prototype 目录的 `package.json`（仅有 `"type": "module"`，目录下无 JS 代码，无实际作用）。
...
- **引擎间 import 可能引入循环依赖**：`workflow-fsm` import `workflow-loader`，如果 loader 反过来依赖 fsm 会出问题。→ 检查依赖方向：loader 是底层（被 fsm 依赖），不应反向引用。subagent 不 import 任何其他 engine，无循环依赖风险。
...
- `agentic-queue-cli.mjs` 搬到 `DPT_FRAMEWORK/cli/operate-queue.mjs` 后，未来是否合并进 `ds.mjs`？这是另一个 OpenSpec change 的事。
