...
The pure resolver owner is `DPT_FRAMEWORK/engine/work-unit-assignment-contract.mjs`, conceptually exposing one operation that accepts registered kind, the hash-bound queue-item snapshot, an already-resolved canonical Topic binding when required, and the Engine-selected/recorded assignment version, then returns the strict merged output contract or a closed rejection. It owns mode/receipt parity, reserved-selector rejection, required-output derivation and base-kind-contract merge/reconstruction. It does not read candidate bytes, mutate queue/index state, allocate IDs, write envelopes, or emit lifecycle actions. Claim and every current first-acceptance reconstruction call this same owner; the target-level `helpers/direct-output-contract.mjs` does not absorb resolver or queue semantics.
...
registered kind
  ...
  + Engine current assignment_contract_version at claim / recorded index version at reconstruction
...
Operation-specific admission, not generic queue storage parsing, requires assignment_mode for every newly enqueued/current claimed `topic_deepening` item and validates mode/receipt parity. Thus old queue files remain readable for explicit repair, while no old live item can be claimed through intent inference. When claim plans a batch, it resolves and validates every candidate in the planned contiguous batch on side-effect-free queue/index views before entering `withWorkUnitTransaction`, allocating any work ID, opening a batch, moving queue entries, or writing any envelope. A normal invalid-batch rejection therefore creates no work-unit lock or transaction record and emits no claim-success event; it may emit only the existing non-authoritative rejection diagnostic. After entering the transaction, claim reloads the queue/index and requires the planned prefix identities and snapshot hashes to remain exact before the first mutation. Concurrent drift at that second check may leave the existing failed-transaction diagnostic, but still creates no work-unit record, envelope, batch-counter change, queue move, or success event. One invalid later item rejects the whole planned mutation.
...
For result-schema projection, the existing `output_files.items` object remains the common optional-output item schema. Each required output contributes one draft-2020-12 branch under `output_files.allOf[]`; that branch contains `contains` with the exact path and role plus `minContains: 1` and `maxContains: 1`. The item schema receives a corresponding `items.allOf[]` `if`-path/`then`-role constraint so an item naming that path cannot advertise another role. `output_files.default` is the ordered `required_outputs` path/role projection, and the Result Starter consumes that same default rather than rebuilding the pairs. Empty contracts retain the existing item schema, use empty `allOf` arrays or omit those no-op arrays consistently, and use `default: []`. Structural schema tests prove this generated shape; formal Zod/candidate validation remains the acceptance owner, so apply does not add Ajv or a hand-rolled general JSON Schema evaluator.
...
| first normal submit | yes, fresh after all binding checks | yes for current marker | successful normal completion owner; rejected formal attempts may retain only the existing `last_submit_rejection`/status/index/transaction/trace/log diagnostics | this invocation's snapshots passed before success mutation |
| eligible first late-submit | yes, fresh after eligibility/binding checks | yes for current marker | audited late completion owner; rejected late validation retains only its existing diagnostic boundary | this invocation's snapshots passed before late acceptance |
...
The candidate adapter combines neutral roots with Engine-validated lifecycle receipt state and returns one non-persistent `recommended_action` plus `primary_root_code`. `DPT_FRAMEWORK/schema/contracts/work-unit.mjs` owns the closed projection schema, while one pure `DPT_FRAMEWORK/engine/work-unit-candidate-projection.mjs` helper owns action derivation and timeout mapping; dry-submit, formal rejection and timeout import those owners rather than redeclare strings. The candidate collector attaches a closed phase ordinal to every violation. Ordering is phase ordinal, then required-output manifest order for direct targets or normalized JSON pointer/coordinate within a phase, then original local issue order as the final tie-breaker. `primary_root_code` is null for submit; otherwise it is the first violation under that deterministic ordering among the highest-precedence scope present, so the Agent never selects a reason code from prose or object-enumeration accidents:
...
Precedence is the table order after `submit`: integrity masks unreliable downstream action; semantic outranks independent declaration cleanup. The projection does not mutate state or perform the action, and formal rejection does not persist the projection inside `last_submit_rejection`. Formal-submit rejection reuses the same action derivation and still points to dry-submit for re-evaluation where applicable. Markdown explains how to execute the named existing operations but does not derive the action itself.

Timeout-preflight keeps its accepted coarser action vocabulary. When it actually evaluates a candidate, it also returns the exact shared candidate projection as `candidate_projection`; when prerequisites prevent candidate evaluation, that field is null. It maps candidate actions deterministically: `submit -> submit`, `repair_same_candidate -> repair`, `return_to_actor -> repair` with actor-owned advice, `fail_and_replace -> block` with the explicit fail/replacement boundary, and `inspect_contract -> inspect`. An independent timeout prerequisite/integrity root may still make the outer timeout action `block`; it does not alter the retained candidate projection or the candidate-action mapping. Timeout must not discard primary_root_code, invent a candidate projection, collapse post-work_done semantic failure into Phase Agent same-candidate repair, or use any candidate action as timeout eligibility by itself.
...
- add/focus `DPT_FRAMEWORK/engine/work-unit-assignment-contract.mjs` as the pure owner of assignment mode/receipt/selector resolution and merged-contract reconstruction used by claim and first-acceptance checkpoints;
- add/focus `DPT_FRAMEWORK/engine/work-unit-candidate-projection.mjs` as the pure owner of candidate action derivation and timeout mapping, with its closed data schema remaining in `schema/contracts/work-unit.mjs`;
