...
- **BREAKING**：current Engine 的 shared reference-binding adapter 只接受当前 UID
  ...
  `related_topic_uids` subset）。仅有历史 `related_topic` 的 reference 一律返回一个
  adapter-owned unsupported-current-contract binding result。
...
- 保留历史 Markdown 的字节、路径和人工可读性；不执行 mass rewrite、migration、copy、
  convert 或自动 repair。拒绝结果指出 reference metadata 这一直接 repair boundary，
  但不授予 Engine 对历史证据的改写权限。
...
| Candidate capability path | Evidence read | Decision | Reason |
|---|---|---|---|
...
| `verification/experiment-*` | catalog | Excluded | C5b owns retained experiment-history policy and remains out of scope. |
...
- `research/canonical-topic-state`: restrict current reference-binding reader
  acceptance to UID forms and define the legacy-only rejection conclusion.
...
- `research/research-wave-gate-implementation`: make legacy-only binding
  non-countable and prevent it from satisfying current Gate/provenance paths.
...
- `workflow/rerun-topic-integration`: keep rerun authoring UID-only and refuse
  legacy-only references as current rerun evidence without rewriting them.
...
- Existing run bundles containing only `related_topic` references remain
