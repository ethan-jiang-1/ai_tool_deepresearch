...
The modified requirements below are limited to content-delivery scenarios and their business assertions. Shared command-experiment mechanics such as disposable-bundle creation, cost metadata, trace-backed verdicts, health/cleanup policy, runner reporting, and current/retired case classification remain owned by the accepted `agent-testing` and `playbook-runner` capabilities plus `guidelines/command-experiments.md`; this delta does not restate a second runner or fixture standard.
