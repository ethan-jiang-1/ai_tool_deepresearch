...
- 为每条active blocking Gate rule补静态`blocking_basis`与`repair.owner/write_to`契约，由`schema/contracts/gate-definition.mjs`中的一个Zod schema统一解析，并扩展GSK-011 active-rule/preflight-root inventory/audit。Runtime loader、consistency validator、post-final rerun guard、work-unit hygiene validator与audit共享这个parser；post-final仍可hash raw bytes，但语义读取不保留`JSON.parse`旁路。`target`只是被检查authority，不自动等于可写面；status/trace/ledger/index/receipt/hash必须指向合法Engine operation或missing-contract boundary，不能建议手改。新Gate/rule或formal-only preflight root若没有hint lineage、producer/authority/checker/test closure或合理blocking basis，静态审计直接失败。
...
Direct Source of Record：`rb_plan.md#/topic_registry`、UID-bound seed、`rb_queue.json`、`_work_units/_index.json` + manifest/beacon/receipt/result、Engine-written `rb_output_declarations.jsonl`、submitted source/cache declarations、reference metadata + canonical topic resolver、accepted `_INDEX.md` table、profile/Wave0 source authority，以及定义静态Gate contract的active Gate definitions/rule/preflight inventory。Checker读取这些direct authorities后产生shared root findings；它们与`hints[]`都只是feedback projection，不是runtime authority。Declaration recovery不增加ledger副本；normal重建使用现有direct owners，late-submit只补不可推导的minimal context。

Net simplification：删除/合并count内容启发式、blocking Key Facts数量、question-list固定顺序、depth-review ledger copies、多处raw topic-field reader、invalid index/declaration cascades、current-output-only source-ref假设、十个Gate wrapper的opaque/hand-built failure、target-string猜修复、error-string root priority和inspect-prose attempt比较；复用normal Topic pipeline、现有queue/work-unit/dry-submit/submit、ledger、topic-layout resolver、现有`wave-contract-findings`、shared Gate result builder与GSK-011 inventory。新增rule repair metadata和一个`recover-declaration` operation；不新增平行failure shape、Gate、persistent hint state、ledger副本、controller或success authority。
...
- Framework implementation：现有topic renderer/layout resolver、queue/work-unit envelope/index/validation/submit/inspect、ledger/provenance/depth/reference/count helpers、一个共享Zod Gate-definition contract及其全部production readers、`wave-contract-findings`、`gate-helpers-core`、active Gate definitions与rule/preflight inventory、十个Gate wrappers和Wave inspect；只新增work-unit owner内的狭窄declaration recovery operation，不新增controller/daemon/retry tree。
...
- Governance/release：更新GSK-002/003/004与EEX-001 registry过时摘要，补齐十Gate definition/CLI/rule/preflight inventory；运行两项governance checks、strict OpenSpec validation、v0.28 release同步。
