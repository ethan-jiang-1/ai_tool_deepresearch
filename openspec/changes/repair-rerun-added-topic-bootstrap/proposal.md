...
更上位的系统性问题是：当前所有正式 Gate CLI 掌握 rule、schema、checked authority、next-action responsibility 和 checkpoint 血缘，却通常只返回 `passed`、松散 `inspect[]` 和泛化 `advice[]`。Markdown Controller 不知道合法写入面时只能猜字段、猜文件、猜命令；Agent连续修一个错再暴露下一个错，是Engine信息保留和Gate contract设计问题。继续逐BUG补failure message只会让严苛Gate与补丁一起增长。本Change因此同时治理全部十个正式Gate CLI：删除没有blocking依据的规则，并让每个primary failure返回Engine已知的合法修复hint。
...
- 治理全部十个正式 Gate CLI，而不只Wave1现场：每个independent primary failure统一输出top-level `hints[]`，包含stable `rule_id`、explicit `repair_kind`、`missing_fact`、exact next-action coordinate `write_to`和exact same-Gate `rerun`；pass时为空。`repair_kind`取`agent_action|engine_operation|user_decision|external_action|missing_contract`，只投影合法下一动作的责任类别，不创造permission；`write_to`按kind分别指授权写入面、Engine operation、决策面、外部前置条件或缺失contract边界。Invalid input、definition/binding、handoff/status preflight、topic-state prerequisite和rule failure都走同一failed-result builder。现有`inspect/advice`兼容保留，但不再是唯一导航。
- 为每条active blocking Gate rule补二选一finding-source契约，由`schema/contracts/gate-definition.mjs`中的一个Zod schema统一解析：单一root contract的declarative rule使用`finding.source: definition`并声明粗粒度closed `blocking_basis`与`repair.kind/write_to`；可能产生多种root的provenance/depth/reference/status等checker使用`finding.source: checker`，由检测到具体direct fact的helper返回root-specific basis、repair kind和next-action coordinate。Basis只回答“为什么可blocking”，精确事实留在`missing_fact`。`$checked_target`只可作为`agent_action` singular non-glob target的显式复用标记，且必须为current finding instance解析成唯一exact coordinate；它不是从target自动猜权限。任一blocking result若缺root-specific basis或repair，shared builder必须改报`configuration_integrity` missing-contract root并mask原symptom，禁止回退到target或`failure_message`。Runtime loader、consistency validator、post-final rerun guard、work-unit hygiene validator与audit共享这个parser；post-final仍可hash raw bytes，但语义读取不保留`JSON.parse`旁路。Audit从parsed definition、definition/CLI自动枚举、现有Gate执行与shared projector证明closure，不再维护逐rule的producer/authority/checker/diagnostic/test副本。Definition外的invocation/config/binding/handoff/status/routing/durability root继续由检测事实的现有helper直接返回同一structured finding，不建设中央root catalog。Status/trace/ledger/index/receipt/hash只能使用`engine_operation`或`missing_contract`并指向合法Engine operation/boundary，不能建议手改。
...
- 把Gate definition的`failure_message`降为definition-file兼容字段：shared projector不得从它生成repair、action-bearing `advice[]`、primary root、failed-rule identity或attempt trend，也不建设解析prose的“非矛盾审计”。旧message无需逐条重写；本Change触碰时可顺手澄清，但机器closure和Controller action只信结构化metadata与checker finding。Wave inspect的missing bundle、definition/config failure也输出统一JSON与`hints[]`；未知bundle只给required-argument command template，不伪造absolute path。
...
- Apply 按green verification islands推进：baseline只锁定当前truth；Gate schema/finding先synthetic、metadata先additive、全部definition合法后再切production parser；quality、Wave0 producer、Wave1 lineage、declaration recovery各自完成focused regression与demand-side wiring后才进入下一slice。禁止把全部未来测试前置成无法顺序完成的RED墙，也禁止靠后续domain改动掩盖前一slice regression。
...
Direct Source of Record：`rb_plan.md#/topic_registry`、UID-bound seed、`rb_queue.json`、`_work_units/_index.json` + manifest/beacon/receipt/result、Engine-written `rb_output_declarations.jsonl`、submitted source/cache declarations、reference metadata + canonical topic resolver、accepted `_INDEX.md` table、profile/Wave0 source authority，以及active Gate definitions和检测definition外root的existing helper contracts。Checker/helper读取这些direct authorities后产生shared root findings；GSK-011从definition schema、实际Gate execution和finding projection派生closure，不另存preflight truth或逐rule contract inventory。Findings与`hints[]`都只是feedback projection，不是runtime authority。Declaration recovery不增加ledger副本；normal重建使用现有direct owners，late-submit只补不可推导的minimal context。

Net simplification：删除/合并count内容启发式、blocking Key Facts数量、question-list固定顺序、Wave2 ledger ordered regex authority、depth-review ledger copies、多处raw topic-field reader、invalid index/declaration cascades、current-output-only/implicit-compatible source-ref假设、十个Gate wrapper的opaque/hand-built failure、现有123-rule test-only artifact-contract inventory、每条复杂rule重复静态basis/repair的风险、central root catalog风险、target-string猜修复、error-string root priority、failure-message semantic regex和inspect-prose attempt比较；复用normal Topic pipeline、现有queue/work-unit/dry-submit/submit、ledger、topic-layout resolver、现有`wave-contract-findings`、detecting helpers与shared Gate result builder。新增rule finding-source metadata、kind-contract prior-role字段和一个`recover-declaration` operation；不新增repair registry、平行failure shape、Gate、persistent hint state、ledger副本、controller或success authority。
...
- `gate-skeleton`: 全十个正式Gate统一root-first `hints[]`、rule finding-source contract、root-owned basis/repair finding、failed-result builder和definition/CLI derived audit。
...
- Framework implementation：现有topic renderer/layout resolver、queue/work-unit envelope/index/validation/submit/inspect、ledger/provenance/depth/reference/count helpers、一个共享Zod Gate-definition contract及其全部production readers、`wave-contract-findings`、`gate-helpers-core`/handoff helpers、active Gate definitions与derived/no-bypass audit、十个Gate wrappers和Wave inspect；删除test-only逐rule inventory，root basis/repair由definition或detecting checker二选一拥有，只新增work-unit owner内的狭窄declaration recovery operation，不新增repair/root catalog、controller、daemon或retry tree。
...
- Governance/release：更新GSK-002/003/004/011与EEX-001 registry过时摘要，把十Gate definition/CLI/rule closure改为schema/behavior-derived audit并保留preflight no-bypass边界；运行两项governance checks、strict OpenSpec validation、v0.28 release同步。
