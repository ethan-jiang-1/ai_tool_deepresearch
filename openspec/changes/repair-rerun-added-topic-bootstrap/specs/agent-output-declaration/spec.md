...
- For a legacy pre-unified-timestamp/pre-context attempt, recovery MAY restore a row only when remaining direct surfaces plus original submit trace/transaction evidence reproduce one row whose hash already equals the submitted index/status hash. If an actually submitted attempt cannot be reconstructed exactly, recovery SHALL block with `repair.owner: missing_contract`; it SHALL NOT offer external backup copying, manual row construction, a replacement attempt, an invented timestamp/audit field, index/status hash rebinding, or a second recovery-row schema as though an accepted operation existed.
