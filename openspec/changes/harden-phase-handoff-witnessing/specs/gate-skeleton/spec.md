...
- the prior deterministic gate has a `gate_attempt` trace event with `passed: true`; and
- the current phase node has a `load_complete` trace event proving the Agent consumed the prior gate's `check.next` through `enter-phase` or the same loader path.

The preflight SHALL derive lifecycle membership and deterministic incoming edges from `manifest.json` and `transitions.chain.json`; it SHALL NOT infer lifecycle membership from file names alone. The instantiation entry phase is exempt from the prior-handoff preflight because the runtime bundle may not exist before instantiation.
...
- **AND** `rb_trace.jsonl` lacks a prior `gate_attempt` with `gate: "wave0-complete"` and `passed: true`
- **THEN** the gate SHALL return `passed: false`
- **AND** `inspect` SHALL identify the missing prior gate pass

#### Scenario: Gate fails when current phase was not entered

- **WHEN** `check-gate-wave1-complete.mjs` is called for `phases/phase-wave1.md`
- **AND** the prior gate passed
...
- **WHEN** the prior gate pass and current `load_complete` witness are present
...
When a gate passes after the fatigue threshold, the gate result SHALL include stop-mode-safe positive advice: `check.next` must be consumed through `enter-phase`, final delivery happens at `phase-final`, and high gate friction does not authorize premature chat synthesis.
...
#### Scenario: High-attempt pass emits continuation advice
