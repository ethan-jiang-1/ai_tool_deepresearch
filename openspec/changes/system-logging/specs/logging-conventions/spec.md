...
定义系统级日志约定——跨 `.mjs`（JS engine/CLI）和 `.md`（Agent phase node）的诊断记录规范。在已有 `logger.mjs`（LOG-001/002/003）和 `trace.mjs`（TRW-001/002）的基础上，建立"什么时候记、记什么、记到哪、用什么 level"的统一规则，使每次 run 的行为可追溯、可排障。
...
所有写入 `rb_trace.jsonl`、`_logs/run.log`、`_trace_subagent.jsonl`、`_trace_agq_cli.jsonl` 的入口 SHALL 包含 `bundle` 字段，使四个 sink 可按 `bundle` + `ts` 缝合为单条时间线。
