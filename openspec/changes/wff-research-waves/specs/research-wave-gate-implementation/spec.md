...
- **AND** `count_floor` 和 per-topic check SHALL 在 topic 集合为空时直接 fail（inspect 指向空 registry），不展开任何 `{topic}` 占位符实例——与 design D5 一致
...
实现 SHALL 支持 `{topic}` 占位符展开（与 wave0 CLI 一致，design D5）：CLI 在 rule iteration 阶段读取 `rb_plan.md` 的 `topic_registry`，对 `target` 含 `{topic}` 占位符的 rule（如 `artifacts/wave1/{topic}/skeleton.md`），为 registry 中的每个 topic key 展开为独立 check 实例。`topic_registry` 为空时所有含 `{topic}` 的 rule 直接 fail。
...
Wave gate CLIs SHALL 延续 `wff-pre-research` 中通过 PRG-007 和 PRG-008 建立的双 trace 约定：
...
- Playbook thin driver SHALL 负责调用 gate CLI、解析 result、通过 `DPT_FRAMEWORK/engine/trace.mjs` 向 `_trace.jsonl` 追加 `event: "check"` trace entry
