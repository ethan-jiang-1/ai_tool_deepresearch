...
- [ ] 1.3 @impl DEW-008, RET-006: Refresh the focused stale-token audit outside `openspec/changes/archive/`, including retired-only and context-sensitive token families, and update `surface-inventory.md` before editing target surfaces.
- [ ] 1.4 @impl DEW-008, RET-006: Classify every inventory hit as `migrate`, `remove`, `negative`, `deprecated-registry`, `checker-self-reference`, or `release-history-minimized`; no `unknown`, `legacy/backlog`, or unreviewed row may remain at archive.
...
- [ ] 2.10 @impl FRE-001, AGT-003, AGO-003: Clean stale Purpose text in `framework-engine`, `agent-testing`, and `agent-output-declaration` where it still implies subagent relay mechanisms, old simple/medium/complex experiment taxonomy, or relay slot output authority.

## 3. Hygiene Gate

- [ ] 3.1 @impl RET-006, DEW-001, DEW-008: Extend `validate-work-unit-hygiene.mjs` to scan current specs, active deltas, framework surfaces, shared experiment infra, tests, guidelines, governance metadata, and `experiments_playbook`, while excluding `openspec/changes/archive/`.
- [ ] 3.2 @impl RET-006, DEW-008: Add allowlists only for explicit negative tests/diagnostics, deprecated registry entries, checker/test self-references, or minimized release-history wording; stale positive relay/slot production wording SHALL fail.
- [ ] 3.3 @impl RET-006: Add regression tests proving archive exclusion, current-surface detection, and allowed negative/deprecated/checker-self-reference/minimized-release-history contexts.
...
- [ ] 3.5 @impl RET-006, DEW-008: Treat `runtime_receipt_ref` and other current work-unit fields as context-sensitive rather than retired-only; fail them only when paired with old relay/slot examples or fields.
...
- [ ] 4.2 @impl AGT-003, DEW-001: Triage playbooks that still rely on `drive-relay-slot`, `subagent-relay`, `_subagents/wave*`, `slot_result_ref`, `subagent_slot_presence`, `slotKey`, `roleAgentKey`, `relay_commit_*`, `relay_spawn_*`, `dispatch.json`, or retired relay helper APIs; migrate those with current work-unit proof value and remove those that have lost current proof/diagnostic value.
...
- [ ] 4.4 @impl PLR-003, AGT-003: Remove the current Legacy/backlog table as a permanent runner surface; migrate valuable cases into current Light/Standard/Heavy tables and remove obsolete cases/files or entries from current surfaces.
...
- [ ] 6.5 @impl DEW-001, DEW-008: Run focused stale-token audit outside `openspec/changes/archive/` for `drive-relay-slot`, `subagent-relay`, `_subagents/`, `slot_result_ref`, `subagent_slot_presence`, `relay slot`, `slot task`, `slot result`, `slotKey`, `roleAgentKey`, `receiptNonce`, `dispatch.json`, `relay_commit_*`, `relay_spawn_*`, `stageSubagentSlots`, `commitSlotResult`, `collectAndMergeSubagentResults`, `recordAgentSpawnRequested`, `ingestAgentReceipt`, and `runProvenanceForensics`; classify every remaining hit as allowed or fix it.
