...
The `available` and `unavailable` branches SHALL support a fixed-width source-class envelope describing the run's current reachability. The envelope SHALL be bounded by a statically declared source-class set and SHALL NOT grow with attempt count:

- each entry SHALL identify one declared source class and one closed-enumeration reachability result for that class;
...
The `unavailable` branch SHALL support one optional routeable classification recording the boundary that owns the failure and the extent of that boundary:
...
- both values SHALL be present together or absent together; and
- an `unavailable` observation without this classification SHALL remain schema-valid and SHALL be treated as explicitly unclassified rather than as any particular boundary.
...
#### Scenario: source-class envelope stays fixed width

- **WHEN** an observation records one closed-enumeration reachability result per declared source class
...
#### Scenario: routeable classification is paired and closed

- **WHEN** an unavailable observation records a boundary-location and boundary-extent value
- **THEN** ProfileSchema SHALL accept only values inside the two closed enumerations
- **AND** it SHALL reject a boundary location without its extent, an extent without its location, and either value on an available or unprobed observation

#### Scenario: unclassified unavailable observation remains valid

- **WHEN** an unavailable observation carries a non-empty reason but no boundary-location or boundary-extent value
...
#### Scenario: legacy observation remains readable without envelope or classification

- **WHEN** a legacy available or unavailable `research_access` observation has no source-class envelope and no boundary classification
- **THEN** ProfileSchema SHALL remain readable for compatibility
...
- **WHEN** `research_access.status` is `unprobed` with timestamp, URL, outcome, reason, tool-surface, candidate-count, candidate-ordinal, source-class envelope, or boundary classification fields
