...
- Preserve the existing source-level Wave evaluator mask as a minimal independent primary repair surface. A prerequisite root records only its downstream rule IDs in durable mask context; it does not recreate them as a primary repair wall. Independent roots remain separately visible.
- Add one schema-parsed rule field, `degradation_eligible`, defaulting to `false`; only the Wave0/Wave1/Wave2 formal adapters consume it. A degraded candidate requires an unmasked, definition-owned `required_floor` finding whose stable `rule_id` exactly matches a parsed Wave rule marked eligible. Queue, receipt, provenance, binding, required structure, trace, lifecycle, checker-owned, and other authority roots remain fail-closed.
...
- Keep formal Gate and side-effect-free inspect as equivalent projections of the same pure evaluator contract. Each command evaluates it once; neither receives a second grouping algorithm, a new verdict, automatic downgrade, partial advance, or recovery controller.
...
- Expected implementation surfaces are the Gate rule schema, a narrow pure Wave degradation-eligibility helper, the shared Wave evaluator/projection, Wave0/Wave1/Wave2 formal adapters and definition metadata, focused root `tests/` coverage, a deterministic disposable-bundle regression, one inactive Wave2 definition fixture, and the `v0.47` release projection. No dependency is added. Other Gate adapters parse the defaulted field but gain no behavior or definition migration.
...
- Net simplification replaces per-Wave eligibility sets, `split(':')` recovery heuristics, and broad primary-hint expansion with one exact-ID eligibility helper and local parent guards. It explicitly avoids a dependency-graph engine, a generic Gate runner, a second validator/verdict, automatic mutation/retry, durable grouping state, a controller, or a human confirmation route.
