...
- 在 `DPT_FRAMEWORK/host_tools/` 增加一个 repo-owned Claude Code launcher，读取 repo 根目录被忽略的 `.env`，将明确配置的本地 Anthropic-compatible DeepSeek endpoint、凭据和模型别名投影到单次 `claude` 子进程。
...
- 用 fake `claude` executable 和临时非敏感配置做 focused integration verification，覆盖环境隔离、local-only 拒绝、redaction、参数/退出码透传；测试不调用真实模型、不需要真实 token。
...
- **最短合法闭环**：`user 配置 root .env -> Agent/用户运行 launcher --check -> launcher 清理并验证单次进程环境 -> exec claude + 原参数 -> Claude Code 连接本地 DeepSeek -> 原样返回退出码`。检查通过后没有额外确认、配置生成、settings 写入或 wrapper orchestration。
