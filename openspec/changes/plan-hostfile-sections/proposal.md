...
- **Template 重写**：`rb_plan.md.tmpl` 从 2 个空占位符扩展为 5 个 section（`## Goal` / `## Topic Registry` / `## Constraints` / `## Progress` / `## Decisions`），含 marker 约定（required-fill vs intentionally-allowed）。Frontmatter 从 JSON 切 YAML。
...
- **更新 `new-disposable-bundle.mjs`**：从 `rb_plan.md.tmpl` 生成 plan body（YAML + 5 section），不再内联 JSON + 1 行 body。消除 disposable vs production 的模板分裂，让 placeholder FAIL 路径在 controlled E2E 可测。
...
- `plan-hostfile-sections`: `rb_plan.md` body 从无结构自由文本变为有明确 ownership 的 5-section host file。Agent 知道往哪写（`## Goal`），sub-agent 知道从哪读边界（`## Constraints`），人和未来的 Agent 能追溯决策链（`## Decisions`），reground 有北星锚点。
...
| `DPT_FRAMEWORK/rb_templates/rb_plan.md.tmpl` | 完整重写（YAML + 5 section + marker 约定 + Progress checkboxes） |
