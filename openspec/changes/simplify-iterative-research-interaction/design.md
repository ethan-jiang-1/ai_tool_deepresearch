...
- 让 existing style CLI 与 C5 lineage validation 复用一个纯 style computation，并让 C5/reentry 复用一个 stage/owner result，既接受合法机械投影又不放宽其他 profile drift。
...
第二种形状闭合既有 phase 顺序中的真实崩溃窗口：topic commit 与 style CLI 已完成，但 count increment 尚未写入。仅当 current exact projection 与 event-bound params 语义不同且 current params 精确等于该 projection 时，这个进度才能从直接值证明；它继续使用现有 `synchronized_initial_profile` stage，不增加第六个 stage，C5 的既有 `current_owner` action kind 只把最近动作指回 phase-rerun 的 count increment。若 exact projection 与 event-bound params 恰好相同，则值本身不能证明 style step 已执行，第一种形状保持 existing topic-state/phase-rerun owner，让既有幂等流程重新 inspect/style 后继续。第三种形状才交给 formal rerun-ready Gate。
