...
- 在 HITL2 推荐或接受 rerun 前，就近用 current profile count（含 rerun phase 必需 increment）与同一个已解析、shape-supported active rule 排除已知不可执行路径；`phase-hitl2.md` 通过只读 Node ESM 调用直接 import 现有 loader，不新增 checker/helper/CLI/state 或复制数值。该判断只是 decision-point advice，正式 legality 仍由现有 rerun-ready Gate 裁决。
...
- 增加 focused unit/integration 验证和真实 `agent_flow_e2e`，覆盖 Agent 从 production guidance 生成 recommendation、HITL1 自然语言接受后进入一个 bounded silent segment、HITL2 自然语言 rerun，以及用户主动回合得到回应但不成为 lifecycle/permission 的边界。
...
- `delegated-work-units`：成功 claim 的 continuation projection 输出 `do_not_initiate`；dry-submit boundary 也服从 lifecycle interaction contract，仍不改变 work-unit 状态、验证、修复或轮询义务。
...
- **Direct Source of Record**：HITL durable decision 仍是 `rb_profile.yaml` + 现有 HITL Gate；phase 交互位置唯一服从 manifest/frontmatter `stop`；`repair_kind` 只提供行动责任/最小边界；路由仍是 transition table + Gate `check.next`；rerun limit 仍是 `DPT_FRAMEWORK/schema/gate_definitions/gate-rerun-ready.definition.json`。Continuation cue 只是这些 direct facts 的无状态 projection。
- **最短合法闭环**：`HITL1 Agent recommendation -> user intent -> existing profile/topic owners -> silent autonomous execution -> HITL2 review + one available recommendation -> user intent -> existing profile decision/Gate/route -> Final or rerun`。Rerun availability 直接读 current count + existing loader 返回的 supported active rule；不可能时留在 HITL2 只问 new-bundle 决定。
- **Net simplification**：删除统一二次确认、固定轮次轻推、文本暗号伪状态，减少内部 enum/phase 暴露，删除“用户不存在”的过度 silent 规则，消除 producer/consumer 两侧的 hint 自动 escalation、shared override table、header/cue 冲突和 rerun-limit/历史版本副本；不新增 state、Gate、CLI、transition、checker、controller、guideline 或 retry/recovery tree。
...
- 影响面集中在 `DPT_FRAMEWORK/workflows/`、continuation projection helper/调用方、Gate fatigue advice、Agent-facing docs、focused `tests/`、`experiments_playbook/` 与 OpenSpec governance/version 描述。不新增 npm 依赖，不修改 runtime schema 或 bundle migration。
