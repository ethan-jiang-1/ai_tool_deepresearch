...
- [ ] 1.1 @impl SNC-006, AGQ-023: Read `proposal.md`, `design.md`, delta specs, `_backlog/bugs/BUG-066*`, `_backlog/bugs/BUG-067*`, and the non-gate portions of `_backlog/bugs/BUG-069*`; record the entrance-contract audit scope before target-code edits.
- [ ] 1.2 @impl SNC-006, AGQ-023: Inventory every Agent-facing entrance surface: phase queue task-card examples, non-delegated queue complete result examples, work-unit `task.md` generation, spawn prompt generation, `_beacon.json`, `result.schema.json`, kind output/cache contracts, and submit validators.
- [ ] 1.3 @impl SNC-006: Compare generated work-unit schema shape against submit-time validation for registered wave0/wave1/wave2 work-unit kinds; record every drift in apply notes before fixing.
- [ ] 1.4 @impl SNC-006: Compare `output_contract.required_result_fields`, generated JSON Schema `required`, and `WorkUnitResultSchema` default/required behavior; choose and record one consistent semantics for each field.
- [ ] 1.5 @impl AGQ-023: Compare phase Markdown queue task-card/result examples against `QueueDemandItemSchema` and `QueueResultSchema`; record every drift before fixing.
- [ ] 1.6 @impl SNC-006, AGQ-023: Classify any discovered mismatch as entrance-contract scope or gate-alignment scope; fix entrance-contract mismatches in this change and leave gate-only findings for `align-gate-contracts-and-reference-navigation`.
...
- [ ] 2.6 @impl SNC-006: Reconcile generated schema required fields with `output_contract.required_result_fields` and submit parser behavior; update output contract metadata if a field is intentionally defaulted.
...
- [ ] 5.5 @impl SNC-006, AGQ-023: Run focused tests for work-unit lifecycle/envelope/submit and queue hygiene; fix failures instead of weakening contracts.
