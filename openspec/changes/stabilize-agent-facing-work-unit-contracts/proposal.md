...
- phase Markdown 中的 queue task-card JSON 示例和 non-delegated queue complete result JSON 示例；
...
- 先做入口合同审计：系统性盘点 Agent 在 enqueue、non-delegated complete、work-unit claim/submit 前会读到的 entrance contract surfaces，并在 implementation evidence 中记录发现，不能只按 BUG-066/067 的已知实例修补。
...
- 扩展 hygiene/static checks，让 phase Markdown 中的 queue task-card/result JSON 示例，以及已有 hygiene 声称覆盖的 work-unit result / entrance contract examples，纳入 retired queue identity 和明显 contract drift 检查，避免 canonical phase 模板假阴性。
...
- `subagent-node-contract`: generated work-unit `task.md` / spawn prompt / beacon / `result.schema.json` 必须 truthful 地投影 submit-time result/output/source-claim/cache contract，让 Sub-agent 能只靠入口合同 surface 写出可提交结果，并让 submit 在 ledger append 前执行同一份 kind-level output contract。
