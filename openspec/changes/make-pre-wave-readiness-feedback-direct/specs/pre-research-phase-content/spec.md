...
#### Scenario: Missing legal surface before fetch stops at the current candidate

- **WHEN** no native fetch surface is available for a selected candidate and invoking the only fallback requires host permission or a legal surface the Agent does not have
- **THEN** the Agent SHALL record an honest unavailable observation with that candidate's `result_url`, positive count/ordinal metadata, `fetch_outcome: not_attempted`, and the direct no-path reason, then expose only that permission or external prerequisite
...
#### Scenario: Missing fallback permission retains an attempted native outcome

- **WHEN** native fetch was invoked for a selected candidate but returned no real page content, and the only same-URL fallback is not legally available
- **THEN** the unavailable observation SHALL retain the actual native non-success `fetch_outcome`, native `fetch_surface`, that candidate's `result_url`, and positive count/ordinal metadata
- **AND** it SHALL not recast the attempted native fetch as `not_attempted`, skip to a later candidate, or create another fallback tier
