...
For Wave1 topic deepening, every accepted source URL claimed by `evidence-summary.md`, topic reference files, submitted result fields, or `depth-review.yaml` SHALL map to a submitted verified cache trail or explicit degraded-capture record. A topic with more claimed accepted source URLs than mapped cache/degraded-capture records SHALL fail submit, preflight, or gate diagnostics before delegated coverage can pass.
...
#### Scenario: Wave1 claimed source URL requires matching cache trail

- **WHEN** Wave1 `evidence-summary.md` claims seven accepted source URLs for a topic
