...
- Add four implementation-neutral review laws without changing the existing Charter's architecture or reading route.
...
The Charter is the baseline, not a problem to replace. Apply keeps every existing top-level section and reading/routing link. It changes only the following local surfaces:
...
| `Guideline Change Checklist` | Add four review questions and a constitutional-admission test | Existing checklist questions and paired evolution reviews |

The new clarification is intentionally a short addition rather than a second Charter. It names four laws:

1. **Operational completeness:** an accepted cross-boundary blocking obligation identifies direct authority, owner, legal producer/transition, applicable witness, consumer, and legal repair or explicit owner/terminal/missing-contract result.
2. **Bounded public reentry:** an advertised public boundary exposes enough explicit inputs and direct facts for the bounded next legal action; it may be a documented protocol, not necessarily a new CLI or Engine transaction.
...
The constitutional-admission check asks whether a proposed rule remains valid after removing the current BUG, Wave, CLI, file, and implementation names; constrains an invariant rather than a chosen mechanism; rejects a future equivalent failure; admits a meaningful counterexample; and leaves other legal mechanisms available.
...
- [Proof scope is read as erasing diagnostics] -> State that altered or historical evidence remains useful for diagnosis but cannot automatically satisfy a stronger closure claim.
...
1. Before apply, validate the change plan and re-read the three target files against this surface budget.
...
5. Archive only after the accepted main spec is synced; future Wave/Gate changes cite these review laws but define their own concrete behavior.
