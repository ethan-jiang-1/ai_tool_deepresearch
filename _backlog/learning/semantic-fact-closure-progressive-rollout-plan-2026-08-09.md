...
> 创建：2026-08-09  
> 最近更新：2026-08-09  
> 状态：tracking backlog，尚未进入新的 OpenSpec change  
...
| 推荐核心 rollout（Phase 2-5） | 0 | 28 | 未开始；等待下一个真实 affected change |
| 条件性 v2 / 后续扩展 | 0 | 7 | deferred；不计入核心 rollout |

核心 rollout 的 `28` 项中，`26` 项由 Agent 执行，`2` 项是用户语义决定（`2.8`、`5.5`）。用户不承担 proposal 文件编写、命令执行、测试、repair、spec sync 或 archive 操作。

**当前唯一 next action：** 等待并选择下一个真实 affected runtime change，按 Phase 2 进入 Propose / Explore；不为满足本计划单独制造 governance change。
...
- [ ] `2.8` `[User decision]` 审定该真实 runtime change 的语义、范围或风险取舍；不要求用户运行 proposal/check/test 命令。

Phase 2 Gate：proposal、delta specs、design、tasks、verification plan 与 closure record 相互一致，required plan checks 通过，且尚未 target edit。
...
| D3 | 不开 standalone hygiene change；先用现有 v1 角色做两次新增真实 dogfood | 校准后的真实 change 再次因角色挤压漏掉 projection/consumer |
