...
若 standalone TODO 被活跃 plan 吸收而非实现，仍用 `git mv` 保留原文，但在 `_done/_done_todos/README.md` 的“已吸收”表登记；不分配 `DONE-NNN`，也不增加 DONE 计数。
...
```mermaid
flowchart LR
  Q["user-guided controls<br/>活跃 unified plan"]
  HELP["helper-not-tool<br/>北星延后"]
  REG["context-reground<br/>parked"]
  Q -.-> HELP
  REC2["✅ bundle-truth recovery"] --> REG
```

用户控制已经并入活跃 unified plan：它是单 run 的 host-file snapshot，先于 helper 的跨 run 记忆，且不阻塞 P0。
...
P0 可跑性阻塞已清零。当前排序按统一研究闭环 + 恢复能力优先：
...
| 顺序 | 项 | 为什么 |
|------|-----|--------|
| 1 | `research-question-closure-and-evidence-judgment` plan, Change 1 | HITL1 将可选用户控制快照到 `rb_plan.md`，Seed/Wave/Final 通过现有 guidance/task brief 读取；不加 profile/path/manifest/Gate 子系统 |
| 2 | same plan, Change 2 | 让模型已声明的 Wave1 carry-forward target 在 Wave2 有可验证去向；Wave0 不新增问题生命周期，不重做 countability 或 actor contract |
| 3 | final-output-eval | pipeline 后段；复用用户控制、问题闭环和可见 limitation，只给 HITL2 recommendation，不自动 rerun |
| 4 | helper-not-tool / hooks / reground | 必须各自先有真实直接缺口；不为预防或测试嵌套机制 |
...
| `research-question-closure-and-evidence-judgment.md` | 用户 per-run 控制 + 问题闭环 | 活跃 plan 的顺序 Change 1/2；前者替代旧 user-knowledge-hang，且先于 helper 跨 run 记忆 |
