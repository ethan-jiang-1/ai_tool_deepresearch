...
| BUG-252 | 2026-09-03 | `operate-work-unit supersede` 对本身是 retry-attempt（attempt_index ≥ 2）的已提交行失败：`buildSupersessionSuccessorDemand` 构造 fresh supersession successor 时只剔除 supersession 字段、保留继承的 `retry_of_work_id`/`retry_reason`/`attempt_index`，`validateSuccessorRetryContinuation` 于是把新 queue identity 下不存在的 attempt-1 父链判为缺失（`retry lineage parent ... is missing`）→ hollow/placeholder 或 hash-drifted 的 attempt-2+ submitted 行没有合法出口；修复：successor demand 构造时清除继承 retry lineage 三字段（supersession successor 是全新 deepening demand），attempt-1 与 attempt-2+ 前置 supersede 行为一致，后续合法 retry 链不受影响（`2026-09-03-supersede-drops-retry-lineage`） |
| BUG-253 | 2026-09-03 | Wave1 真实重跑中多个 dpt-evidence-extractor agent 的 `result.json` 被 dry-submit 拒绝：`cache_trail_refs`/`degraded_capture_ref` 写成 `<leaf>/page.md` 文件路径（应为 cache leaf 目录）、`source_ref` 写成 slug（应为当前 assigned output 或授权 prior output）、同一 URL claim 复制多份且 `accepted_source_urls` 脱节、claim.url 与 leaf meta.json url 差异——生成指引/诊断未把这些值域一次讲清；修复：task.md「Cache And Source Facts」明示值域 + accepted/degraded 正例（DEW-030），`validateSourceClaims` 对已失败的 accepted claim/URL root 附带重复 claim URL 计数与指针范围（enrichment，不改 verdict，DEW-031）（`2026-09-03-wave1-result-authoring-contract`） |
| BUG-254 | 2026-09-04 | run-scoped 脚本把中间产物硬编码写系统 `/tmp/`（114 个 `/tmp/` 文件现场取证），不随 bundle 归档、跨环境丢失、provenance 断链，且与 `08_haiguang-shensuan4-dcu/source.yaml` 前缀 drift 不可逆有直接因果；修复：bundle 自带 `_tmp/`（instantiation scaffold + README），脚本中间产物一律写 bundle `_tmp/`，禁止系统 `/tmp/`，`stagingFile(slug, kind)` helper，inspect 只读扫描 `_scripts/*.mjs` 硬编码 `/tmp/` 写路径并报 active-bundle blocker（`2026-09-04-run-scoped-tmp-inside-bundle`） |
| BUG-255 | 2026-09-04 | `audit-phase-status.mjs` trace completion witness 时序检查与 phase 文档/gate 定义死锁：gate 要求 completion 先写（"must write a completion trace event before the gate"），audit 却要求 `gate_attempt.ts <= completion.ts`（gate 先过、completion 后写），导致按正常流程完成的 bundle 首轮 completion 事件被误报 `trace_integrity_unsupported_completion`（3 个 bundle 现场取证，含 `dpt_rb_glm-5-3-deepseek-v4-domestic-chips` 09-04 两条合法 `log-event` 写入的 `wave1_completion`/`wave2_completion`）；修复：witness 检查去掉时序约束，仅验证存在匹配 gate identity 的 passed `gate_attempt`，时序一致性交给 `ts_monotonic`（append 序非递减）；main spec TRW-008 同步、单测/集成正例反例全绿、全量回归 3122/3122（`2026-09-04-fix-audit-completion-witness-timing`） |
| BUG-256 | 2026-09-07 | parseArgs 类 CLI 未处理 `--help`/未知选项，抛未处理 Node 内部 `ERR_PARSE_ARGS_UNKNOWN_OPTION` stack dump、exit 1（`reconcile-plan-progress`/`audit-phase-status`/`check-reentry`/`log-event`/`validate-work-unit-hygiene`/`apply-research-style` 共 6 个，repair playbook 点名的诊断/修复入口全部命中）；修复：新增共享守卫 helper `engine/helpers/cli-args.mjs`（`--` 前出现 help token → Usage + exit 0，未知选项/解析失败 → `invocation error` + Usage + exit 2，域内码不变），6 个 CLI 全部接入，`log-event` 例外收窄为"诊断/日志结果 always-0 + invocation 拒绝 code-2"，main spec CLE-001/003/004 同步、COMMANDS.md 与 cli/README inventory 更新、helper 单测 + 既有 exit-code 回归扩展（batch 运行时探针/静态锁/文档锁）；全量回归 3183/3183 绿（`2026-09-07-unify-cli-help-unknown-option-guard`） |
