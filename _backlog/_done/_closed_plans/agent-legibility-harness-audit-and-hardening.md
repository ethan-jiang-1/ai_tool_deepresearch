...
- **关键反思（诚实记录）**：plan 阶段把「入口链双副本」误判为 borrowing 02/09 的漂移源，apply 前读 ACR spec + 既有 guard test 才发现「两份保留 tool identity + byte-sync guard」已是 accepted behavior 且被机器强制。symlink 方案会违反 ACR，故撤销入口链改动、把 Change A 收敛为纯「归属表」change。**后续补充**：用户（owner）明确决策「维护两套头疼，CLAUDE.md 做成 symlink 即可」，优先于 ACR 的 tool-identity 设计；symlink 方案由此落地为 change `2026-08-25-entry-chain-single-source-symlink`（ACR-002/ACR-004 改为「单一 `AGENTS.md` + `CLAUDE.md` symlink + `check-entry-chain.mjs` 形态检查」+ guard test 更新 + `AGENTS.md` 第三行中性化，finalizer 17/17，`npm test` 2826/2826）。教训：**下诊断前先读该事实的 owner spec + 既有 executable coverage，别把「两份」直接等同于「漂移」；同时 tool-identity 的取舍是用户决策，agent 只报告 trade-off 不替用户定**。
