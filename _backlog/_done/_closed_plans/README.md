...
> 最后更新: 2026-09-04（含补登记对账节） | `_backlog/_done/_closed_plans/` — 已完成 plan 的归档目录。
...
| CLS-086 | 2026-09-04 | spec-lean-pointerization-and-registry-hygiene.md | spec-lean 主线 C1–C5 全落地:C1 `2026-09-04-spec-lean-restatement-tooling`(扫描器+装配器)、C2 `2026-09-04-dwu-capability-identity-split`(DWU 一变四:母体+submission/preflight/correction,registry 719/76)、C3 `2026-09-04-megablock-requirement-split`(11 块≥190→30 子块,纯文本守恒,registry/header 零触碰)、C4 `2026-09-04-registry-hygiene-and-guard-extensions`(§-guard 规则3 + 新 nav 符号 checker,首扫修 8 文件过时注释)、C5 无遗留(M1/M4/M5 已吸收)。四 change 全管线 propose→polish→apply→archive→commit,finalizer 均 19/19;全库无 requirement 块 >190 行。 |

> CLS-086 附:三份卫星文档随主 plan 一并移入本目录(设计依据/参照资料,非独立编号 plan):`spec-lean-capability-split-dwu.md`(C2 DWU 一变四设计)、`spec-lean-f4-megablock-deepdive.md`(C3 逐块处置表)、`spec-lean-capability-study-1000plus.md`(1000+ 行研究)。

| CLS-085 | 2026-09-04 | regression-suite-runtime-profiling-and-speedup.md | 回归套件耗时剖析与提速路线：A/F 路线经 change `regression-suite-tooling-speedup` 落地归档（`2026-09-04-regression-suite-tooling-speedup`，finalizer 19/19）——`npm test` 入口改 `scripts/run-tests.mjs`（`os.availableParallelism()` 并发 + `NODE_COMPILE_CACHE` gitignored 缓存 + `--dry-run`/参数透传），`.gitignore` 加 `/.cache/`，权重表 `scripts/test-weights.json` + regen 脚本（325 文件）；实测修正：c4→c8 仅 -15%（runner 已过订）、arg-order LPT 被 R1 探针证伪（`node --test` 字典序排序参数）按预设分支回退、C 路线前提证伪（snapshot/restore 已普遍落地）；墙钟 281.8s→6 次全量 209.7~276.4s 全绿、中位 ≈226s（回退通过线 ≤240s），总 CPU 1061 CPU-s、8 核下限 ≈133s。采样协议偏离（启动 load 门不可执行）经用户授权以证据收尾。D/E 后续投资转 step2 计划。 |

**Next available plan ID: CLS-087**
