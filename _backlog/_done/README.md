...
> 最后更新: 2026-08-12 | `_backlog/_done/` — 所有已完成内容、已吸收设计输入与已替代历史 TODO 的归档根目录。
...
├── _fixed_bugs/           # 已修复/结案 Bug（193 个，含间隔）
├── _suspened_bugs/        # 悬挂 Bug（16 个：含 BUG-026, 028, 030, 099, 106, 129--131, 142, 144, 145, 147, 149, 161, 163, 175）
├── _done_todos/           # 已完成 TODO（16 个，DONE-001~016）+ 4 个已吸收、1 个已替代 standalone TODO
├── _closed_plans/         # 已完成 Plan（59 个）
...
| 归档目录 | 数量 | Next ID |
|---------|------|---------|
| `_fixed_bugs/` | 193 个已修复/结案 bug（含间隔） | BUG-220 |
| `_suspened_bugs/` | 16 个悬挂 bug（含 BUG-026, 028, 030, 099, 106, 129--131, 142, 144, 145, 147, 149, 161, 163, 175） | — |
| `_done_todos/` | 16 个已完成 todo（DONE-001~016）+ 4 个已吸收、1 个已替代设计输入（非实现） | DONE-017 |
| `_closed_plans/` | 59 个已完成 plan | CLS-060 |
...
- **evidence-extraction 核心计数**（2026-07-02 落地 / 2026-07-09 todo 归档 DONE-015）：countReferences() + isCountable() + Engine-computed ref_count 已在 `ref-count.mjs`；per-run 用户控制已落入现行 `user-research-controls` contract。详见 `_done_todos/todo-evidence-extraction.md`
...
### 想看"现在该做什么"
→ [`../todos/README.md`](../todos/README.md)——当前活跃 TODO、依赖关系与执行顺序
...
→ `_done_todos/` 下 16 个 DONE 文件、4 个已吸收设计输入与 1 个已替代设计输入（按文件名主题查阅）
