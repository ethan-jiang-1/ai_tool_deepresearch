...
This is a deferred historical operability observation, not the current delivery
blocker. A premature stop costs an extra user continuation turn but does not
itself make the bundle's work-unit, evidence-production, or Gate contracts
unreachable. Current deterministic swarm contracts have since converged; do
not reopen a handoff implementation candidate or solve host/Agent turn liveness
by adding a DPT watcher or controller.
...
This card preserves a July incident, not a current-head deterministic defect.
The production path is now `queue demand -> work unit -> sub-agent -> submit
-> ledger -> gate`; stop:no delegated phases have an active poll-submit-repair-
terminalize loop, and actor selection is an explicit native-probe decision.
`case-606` proves continuation-cue visibility only. The later real-actor
canaries reached no native completion and are host-scoped `NOT_RUN`, so they
neither reproduce nor close this card.

Do not treat the historical context-pressure explanation below as proven. The
next valid evidence is a fresh current-head Phase-Agent observation with the
completed handoff, bundle trace/status, prompt/transcript, host/version/mode,
and the first target-phase action retained.
