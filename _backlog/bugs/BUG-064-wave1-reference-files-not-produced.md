...
1. **把 reference 文件加入 `required_receipts`**：task card 模板中增加 `"file:reference/{topic.slug}-<source-slug>.md"` 到 required_receipts
2. **task.md 中显式写 reference 文件的最低数量要求**："at least one reference file per topic, following shared-reference-template.md format exactly"

### 中期

3. **Phase Agent 在 submit 后做 reference 完整性检查**：如果 output_files 中没有 role=reference 的条目，记录 `silent_degradation` 并提示补充
4. **Wave1 gate 增加 per-topic reference file count**：类似于 wave0 的 per_topic_count_floor

### 长期

5. **解耦 reference 文件生成和 sub-agent 搜索**：sub-agent 负责搜索+提取数据，Phase Agent 负责把数据物化为 reference 文件。这样即使 sub-agent 不写 reference，Phase Agent 也可以从 evidence-summary 和 cache 中提取内容生成 reference
