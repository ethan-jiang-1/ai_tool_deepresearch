...
| `guidance-drift-cleanup-machine-guards` | 把深挖发现的漂移收敛为 **3 个 OpenSpec change + 1 deferred**：C1 事实校准（散文说真话+入口单一源+AGENTS≡CLAUDE 守卫）、C2 恢复反馈直达（Engine 自带下一步）、C3 文档-代码漂移守卫（checker+可选本地 pre-commit）。C4 phase 闭包去重 deferred。 | active（C1 CLS-064 ✅、C2 CLS-065 ✅ 均已归档；待 propose C3） |
