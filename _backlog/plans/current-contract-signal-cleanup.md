...
>
> 总进度：`[########------]` **8 / 14 个执行项已归档；还剩 6 个（09-14）**
> 当前位置：**09 / Propose 阶段；本项 4 / 19 步已完成**
> 眼前一步：**完成 `design.md`**
> 修改权限：**尚未获得 APPLY 授权；Harness、tests、accepted main specs 不得修改**

## 总 Todo Checklist

编号 `01-14` 是实际执行顺序，每一项对应一个 OpenSpec 执行批次；`C...` 是审计卡编号，
...
- [ ] **09 `C5a-2`：当前 Engine 拒绝历史 reference binding（当前执行项）**
  - OpenSpec: [`decide-historic-reference-reader-policy`](../../openspec/changes/decide-historic-reference-reader-policy/)
  ...
  - [ ] **创建 `design.md`（下一步）**
  - [ ] 创建 `tasks.md`
  - [ ] 创建 `semantic-closure.yaml`
  - [ ] 创建 `verification-plan.yaml`
  - [ ] 运行 strict OpenSpec 与 planning-governance validation
  - [ ] 完成 proposal review / polish
  - [ ] 用户明确授权 `APPLY`
  - [ ] 首次修改目标代码前完成 plan review
  - [ ] 实施已批准的 tasks
  - [ ] 运行 selected verification 与 current-path regression tests
  - [ ] 将 delta specs 同步进 accepted main specs
  - [ ] 完成 closeout review
  - [ ] Governed archive
  - [ ] Commit implementation / archive artifacts
  - [ ] 更新本看板与 execution ledger
- [ ] 10 `C1b+C1c+C1d+C1e`：移除未使用或仅表述未来承诺的 inactive surfaces
- [ ] 11 `C1f+C5b`：收口旧 experiment-history 输入（满足合并门时合并）
- [ ] 12 `C6a+C6b+C6c`：停止读取历史 work-unit records（满足合并门时合并）
- [ ] 13 `C6d`：将 transaction v1 明确限定为不支持的历史格式（单独执行）
- [ ] 14 `C7+C8`：最终清理 main specs、context 与 routing 的当前态表述

## 为什么是 14 项

- **审计口径：** 21 张细粒度 decision cards，用来保证风险不漏项。
- **执行口径：** 8 个已归档 change + 6 个剩余执行批次 = 目标总数 14。
...
- 失败后果或 mutation safety 不同时必须拆开，所以 `C5a-2` 不与 experiment 合并，
  `C6d` 不与 `C6a-C6c` 合并。
- 条件合并若不成立，只调整尚未执行的分母；已经完成的编号和记录不重写。
...
| 09 的具体提案 | [Active OpenSpec proposal](../../openspec/changes/decide-historic-reference-reader-policy/proposal.md) |
