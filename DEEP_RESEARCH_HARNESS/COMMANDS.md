...
| 工具 | 文件 | 说明 |
|------|------|------|
| operate-artifact-persistence.mjs | cli/operate-artifact-persistence.mjs | 对非-primary staging file 执行 generic CAS + atomic persist；primary Final 只经 `publish-final-report --bundle --source [--feature]` 分配 immutable base/global version、检查 Evidence Map submitted backing 并 no-clobber commit；quiescent `sweep` 复用记录 binding，不重新分配。它不授予 semantic support、submit、gate、handoff、delivery、quality 或 satisfaction authority |
