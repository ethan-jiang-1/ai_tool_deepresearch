...
- `operate-artifact-persistence.mjs` is a three-operation durability command: generic `persist` commits non-Final-Markdown content but exits `1` and names `persist-final-report` for safe Final Markdown targets; `persist-final-report` admits one Final Evidence Map's safe submitted backing before the existing CAS/atomic commit and emits its `check` / `inspect` / `advice` separately from durability verdict; `sweep` exits `0` when every accepted workspace is finalized/cleaned, `1` when any workspace needs Agent action, and reruns Final-backing admission only before finalizing a prepared Final Markdown payload. Invalid invocation/configuration exits `2`. Always read the JSON verdict; sweep requires a quiescent bundle with no concurrent persist.
...
| 工具 | 文件 | 说明 |
|------|------|------|
| operate-artifact-persistence.mjs | cli/operate-artifact-persistence.mjs | 对非 Final Markdown staging file 执行 generic CAS + atomic persist；对 safe Final Markdown 只经 `persist-final-report` 先检查 Evidence Map 的安全 submitted backing，再复用同一 durability path；quiescent `sweep` 在 Final prepared payload rename 前复查该 admission。它不授予 semantic support、submit、gate、handoff 或 delivery authority |
| persist-artifact | command_playbook/persist-artifact.md | Agent copyable 闭环：保留 staging → non-Final `persist` 或 Final Markdown `persist-final-report`；崩溃后停止并发 persist → sweep；blocked 时检查/移除单个 workspace、retry the named operation、rerun sweep |
