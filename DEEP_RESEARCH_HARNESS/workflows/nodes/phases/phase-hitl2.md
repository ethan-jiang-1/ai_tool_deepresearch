...
- 记录 `hitl2_recorded` trace event 到 `rb_trace.jsonl`：
  ```bash
  node DEEP_RESEARCH_HARNESS/cli/log-event.mjs --bundle <path> --event hitl2_recorded
  ```
...
```bash
node DEEP_RESEARCH_HARNESS/cli/gates/check-gate-hitl2-recorded.mjs --bundle <path> --current-node phases/phase-hitl2.md
```
...
- `proceed_to_readiness` → gate CLI 使用 deterministic `passed` outcome，`check.next` 应为 `phases/phase-readiness.md`。先消费 selected handoff，再同步 source gate status：
  ...
  node DEEP_RESEARCH_HARNESS/cli/advance-status.mjs --bundle <path> --to hitl2_recorded
  ```
...
| 时机 | 命令 |
|------|------|
| Phase 开始 | `node DEEP_RESEARCH_HARNESS/cli/log-event.mjs --bundle <bundle> --level info --msg "phase:hitl2 START"` |
| Phase 结束 | `node DEEP_RESEARCH_HARNESS/cli/log-event.mjs --bundle <bundle> --level info --msg "phase:hitl2 END — <summary>"` |
