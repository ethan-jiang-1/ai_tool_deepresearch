...
我建议先按下面的**研究对齐草案**开始。它是我对本轮目标、研究对象、决策或交付用途和范围的建议；在你接受、修正或明确委托前，下面的 must-answer、Topic map 和 profile 都还只是建议。

**目标、研究对象、决策/交付用途与范围**：{DYNAMIC: grounded_goal_and_scope}
...
如果我判断存在会实质改变既有研究决定、彼此独立且现在可以回答的研究路线分叉，我会在首轮最多提出三个问题：

{DYNAMIC: material_frontier_questions_or_none}

每个问题都会说明我的推荐或透明默认值，以及它会影响的既有决定（例如 must-answer、Topic map、research profile、明确范围排除、来源/证据约束或交付重点）。依赖另一个未决答案的问题会等前置决定清楚后再提；不改变研究路线的细节会采用透明默认值，不会变成主动澄清问题。

你可以直接说“按这个开始”，也可以用自然语言修正目标、必须回答的问题、话题或研究深度，或明确说“请按你的建议继续”。A/B/C 只是可选快捷方式：
...
清楚的接受、修正或委托即解决当前 HITL1 决定。我会简短重述已解决的研究理解后继续既有路径，不会要求笼统的第二次确认。
