...
@impl HIU-001, HIU-002, HIU-005, HIU-006

HITL1 和 HITL2 是两个有帮助的决策环，不是让用户学习 enum 的问卷或命令控制台。

## 入口

Agent 一次展示当前事实、一个明确推荐、理由与影响、可选快捷方式，并说明用户可以直接接受、自然语言修正或继续提问。推荐不替代用户决定。HITL1 先展示研究对齐草案：目标、研究对象、决策或交付用途、范围，以及建议的 must-answer、最小独立 Topic map 和 profile；这些在用户决定前都只是可审阅建议。

只有当一个当前可回答的答案会实质改变既有研究决定时，HITL1 才可主动澄清。首轮可并列展示最多三个彼此独立的问题；每项都说明推荐或透明默认值和会受影响的既有决定。依赖未决答案的问题留到后续，能以透明默认值处理或不改变研究路线的细节不应变成问题。

## 环内

- 清楚接受、修正或明确委托：该表达本身就是决定；重述已解决的研究理解，把已展示且被接受或委托的语义写入当前 HITL owner，运行 Gate 并继续，不要求笼统的第二次确认。
