...
- For `suspect_transaction`, do not wait, force-timeout, delete a lock, or invent cleanup. Run `node DEEP_RESEARCH_HARNESS/cli/operate-work-unit.mjs recover-transaction <bundle> --tx-id <id>` only when `repair_kind: recover_transaction` and exact `write_to` select one unlocked proof-complete v2 `started`/`suspect` journal. Resolve `<bundle>` to the projection's canonical absolute `bundle_dir`. After a changed or idempotent settled result, rerun the preserved original checkpoint. A valid global holder remains `busy`; any legacy, incomplete, unsafe, drifted, or held-suspect proof returns `missing_contract`.
...
Never reactivate the submitted predecessor or manually edit `rb_output_declarations.jsonl`, `_work_units/_index.json`, `_status.json`, `rb_queue.json`, `_work_units/.lock`, transaction journals, `result_hash`, or `ledger_record_hash`. A malformed relation, branched/cyclic lineage, unattributable ledger corruption, or unavailable proof remains `missing_contract` rather than permission to choose or repair authority by hand.
