...
All bare runtime paths in this node are current run bundle-root relative. `_work_units/waveN/{work_id}/`, `rb_output_declarations.jsonl`, `_cache/...`, and `_logs/...` refer to the selected `dpt_rb_*` or `dpt_disp_*` bundle, not repo root or `DEEP_RESEARCH_HARNESS/`. Every claimed attempt also exposes one canonical absolute `bundle_dir`; use it with the absolute paths returned in `prompt_refs[]` rather than reconstructing a root from cwd or a bundle basename.
...
| `operate-work-unit recover-declaration` | Existing-owner restoration for one already-submitted attempt whose exact Engine declaration row is missing; it accepts only `--work-id` and succeeds only when direct owners reproduce the accepted row hash-identically. |
| `operate-work-unit recover-transaction` | Proof-limited reconciliation for one exact unlocked v2 journal; it may mark only that prior journal `rolled_back` when every declared target still matches its before-image. |
| `operate-work-unit supersede` | Audited correction for one eligible submitted predecessor; it preserves predecessor authority and creates one fresh ordinary successor queue demand. |
...
3. For a normal delegated top-up, compute:
   ...
   `eligible_independent_demand` is the queue-front count of independent eligible demand for the phase, and `remaining_free_capacity` is the effective cap minus reconstructed normal delegated in-flight work. For Wave0, independence additionally requires current canonical assignment contracts to resolve to pairwise-distinct exact `source_yaml` targets. Different Topic targets remain batchable, but different task briefs, research dimensions, queue IDs, cache trails, or URL sets do not make two same-target demands independent. Combine one Topic's dimensions into one standard demand or schedule a later serial supplement after the named owner terminalizes or submits. If reconstructed normal delegated in-flight work already reaches the effective cap, poll, submit, repair, or terminalize existing attempts before claiming more. `--count 1` is legal for a single remaining item, dependency- or same-target-blocked front item, effective cap of 1, or a narrow repair; it is not the normal actor drain strategy for independent demand.
...
For `wave0_source_target_conflict`, follow the returned exact `source_target`, named queued or in-flight owner, and same-claim `rerun`. A queued-prefix conflict uses the emitted reduced conflict-free count; an in-flight owner stays on its existing poll, dry-submit, submit, repair, or terminalization loop before claim is retried from fresh facts. Do not rename queue IDs, split cache paths, edit ledger/source-array authority, or seek user permission to create concurrent same-target writers.
...
7. Read the generated Result JSON Starter embedded in `task.md`, then have the selected actor prepare the candidate at the assigned absolute `result_ref`. The starter projects the current schema and checklist; it is not a prewritten result or success authority. The claimed `actor_execution` route plus exact `work_id` and `receipt_nonce` owns candidate authorship: only `phase_agent_fallback` lets the Phase Agent author that exact fallback attempt. For `delegated_subagent`, the Phase Agent may inspect, dry-submit, or formally submit the returned candidate, but must not author substitute delegated content under the same binding. This guidance does not authenticate a physical actor or prove host/sub-agent liveness.
...
```bash
node DEEP_RESEARCH_HARNESS/cli/operate-work-unit.mjs recover-declaration "<canonical-absolute-bundle_dir>" --work-id <submitted_work_id>
```
...
### Attempt Disposition And Recovery

Preserve the exact command/checkpoint that emitted the root. Inspect and submit preflight expose `identity`, `transaction`, `coverage`, and `next` for the exact `work_id`, `queue_item_id`, `receipt_nonce`, logical actor route, assigned result/receipt coordinates, and current ledger relation; this projection is not persistent authority.

- For `busy`, read caller operation/work coordinates separately from holder `tx_id`, holder operation, target work/queue coordinates, `journal_disposition`, and `targets_same_attempt`. Wait, then run the emitted caller-same-operation `rerun` for the same inspect, dry-submit, timeout-preflight, formal-submit, supersede, or transaction-recovery checkpoint. Do not infer actor ownership, progress, process death, or liveness from a valid holder.
- For `suspect_transaction`, do not wait, force-timeout, delete a lock, or invent cleanup. Run `node DEEP_RESEARCH_HARNESS/cli/operate-work-unit.mjs recover-transaction <bundle> --tx-id <id>` only when `repair_kind: recover-transaction` and exact `write_to` select one unlocked proof-complete v2 `started`/`suspect` journal. Resolve `<bundle>` to the projection's canonical absolute `bundle_dir`. After a changed or idempotent settled result, rerun the preserved original checkpoint. A valid global holder remains `busy`; any legacy, incomplete, unsafe, drifted, or held-suspect proof returns `missing_contract`.
- For a submitted declaration root, `recover-declaration` remains the sole nearest action whenever exact reconstruction is reachable. Run its exact command, then rerun the same inspect/Gate. Do not offer `supersede` in parallel.
- Run `node DEEP_RESEARCH_HARNESS/cli/operate-work-unit.mjs supersede <bundle> --work-id <submitted_work_id> --reason <audit-reason>` only when the Engine selects eligible attributable post-submit drift, using the projection's canonical absolute `bundle_dir` for `<bundle>`. Read the returned predecessor work/queue IDs, committing transaction ID, `successor_queue_item_id`, and ordinary successor location. A queued successor returns to current exact-role actor observation and normal claim; an in-flight successor is polled by its existing work ID; a terminal successor follows only its own ordinary contract. Only the unique current lineage leaf's normal submit or audited late-submit restores current coverage; then rerun the same inspect/Gate.

Never reactivate the submitted predecessor or manually edit `rb_output_declarations.jsonl`, `_work_units/_index.json`, `_work_units/waveN/<work_id>/_status.json` (work-unit 级;bundle 级状态是 `rb_status.json`), `rb_queue.json`, `_work_units/.lock`, transaction journals, `result_hash`, or `ledger_record_hash`. A malformed relation, branched/cyclic lineage, unattributable ledger corruption, or unavailable proof remains `missing_contract` rather than permission to choose or repair authority by hand.
