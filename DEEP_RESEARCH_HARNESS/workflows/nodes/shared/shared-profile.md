...
说明 `rb_profile.yaml` 的当前字段、类型、填写时机、示例值和 Source of Record。此 node 是 Agent-readable guidance，不替代 schema 或 current run bundle state。
...
- **填写时机**：HITL1 isolated direct-sample probe 后写入；bundle 可以缺失
- **分支字段**：
  - `unprobed`：仅 `status`
...
- **boundary**：current observation 不推断、不携带 host、network 或 owner boundary。样本 outcome 是 Phase 的语义判断输入，不是 Engine 网络诊断。
...
- **Schema authority**：`DEEP_RESEARCH_HARNESS/schema/contracts/profile.mjs`（`ProfileSchema`）
- **Runtime truth**：当前 current run bundle 的 `rb_profile.yaml`
