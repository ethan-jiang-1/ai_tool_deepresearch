...
```bash
node DEEP_RESEARCH_HARNESS/cli/plan-hostfile-sections.mjs render-no-controls
```

For a resolved snapshot saved at one explicit UTF-8 input path, run:

```bash
node DEEP_RESEARCH_HARNESS/cli/plan-hostfile-sections.mjs render-supplied-controls --input <snapshot-path>
```
