...
//
// Usage:
//   node DEEP_RESEARCH_HARNESS/cli/advance-status.mjs --bundle <path> --to <gate>
//
...
const command = 'node DEEP_RESEARCH_HARNESS/cli/advance-status.mjs';
const usage = [
  'Usage:',
...
if (handoffCheck.covered) {
  if (status.current_node !== nextNode) {
    console.log(JSON.stringify({
      ...
      advice: [`Run enter-phase with the source gate check.next before status sync: node DEEP_RESEARCH_HARNESS/cli/enter-phase.mjs --bundle ${bundlePath} --node ${nextNode}`],
    }));
    process.exit(1);
  }
