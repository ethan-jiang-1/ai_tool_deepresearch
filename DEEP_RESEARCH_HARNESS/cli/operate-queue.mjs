...
// @impl FRE-001: Canonical CLI location DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs
...
function usage() {
  ...
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs enqueue <bundle> --task <task.json>
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs claim <bundle> --actor <main-agent|sub-agent>
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs complete <bundle> --result <result.json>
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs fail <bundle> --failure <failure.json>
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs preempt <bundle> --task <task.json> --reason <reason> [--unsafe-current]
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs count <bundle>
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs render <bundle>
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs project <bundle>
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs repair <bundle> --remove-stale
...
function enqueueAssignmentModeFeedback(admission, { bundlePath, taskPath }) {
  ...
  const rerun = `node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs enqueue ${JSON.stringify(bundlePath)} --task ${JSON.stringify(taskPath)}`;
  return {
    ok: false,
    reason_code: admission.reason_code,
    reason: admission.reason,
