...
const EXECUTION_SURFACE_RUNTIME_ENTRYPOINTS = Object.freeze([
  'DEEP_RESEARCH_HARNESS/host_tools/run-agent-experiment.mjs',
  'DEEP_RESEARCH_HARNESS/cli/inspect-bundle.mjs',
  'DEEP_RESEARCH_HARNESS/cli/validate-bundle.mjs',
  'experiments_env/shared/verify-bundle-health.mjs',
]);
const STATIC_RELATIVE_IMPORT_RE = /\bimport\s+(?:[\s\S]*?\s+from\s+)?['"](\.{1,2}\/[^'"]+)['"]/g;
const NAMED_HELPER_PATH_RE = /\b(?:DEEP_RESEARCH_HARNESS|experiments_env\/shared)\/[A-Za-z0-9._/-]+\.mjs\b/g;
...
function executionSurfacePath(repoRoot, filePath) {
  ...
  if (!fileRelative.startsWith('DEEP_RESEARCH_HARNESS/') && !fileRelative.startsWith('experiments_env/shared/')) {
