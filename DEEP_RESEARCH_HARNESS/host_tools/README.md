...
node DEEP_RESEARCH_HARNESS/host_tools/claude-deepseek.mjs -p "hello"
node DEEP_RESEARCH_HARNESS/host_tools/claude-deepseek.mjs --verbose
```
...
`.exp-bundles/` never contains a copied, symlinked, or hardlinked `DEEP_RESEARCH_HARNESS/`, `experiments_env/`, or `tests/` tree. The Playbook Agent cwd remains the validated repository command root so repo-relative source commands use the one original framework; only mutable run data enters the case run root. This containment is mutation/cleanup authority, not an OS sandbox for hostile playbooks.
