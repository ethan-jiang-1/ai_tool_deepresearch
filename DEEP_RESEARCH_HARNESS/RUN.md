...
- `rb_profile.yaml` now owns the schema-validated
  ...
  normal delegated work-unit prompt fan-out. The Phase Agent derives its
  bounded claim count from that profile value and existing in-flight demand;
  Engine work-unit allocation, actor preflight, queue admission, transaction,
  and fallback authority remain unchanged. This policy does not claim host
  capacity or physical concurrency.
...
- A Wave1 Phase Agent may record the smallest current focus commitments in the
  ...
  commitments must name reviewed, hash-valid submitted Wave1 work for the same
  canonical Topic and current rerun count; visible limitations remain separate.
- Wave1 keeps its existing checkpoint and verdict partition: invalid or
  repairable focus coverage is an ordinary depth-contract failure, while a
  valid `partial` or `blocked` limitation is the definition-owned
  `focus_coverage_limit` under the existing degradation policy. This adds no
  focus-specific route, status, queue kind, retry controller, or checkpoint.
