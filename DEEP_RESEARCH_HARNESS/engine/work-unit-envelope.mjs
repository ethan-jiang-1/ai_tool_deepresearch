...
function taskMarkdown(manifest, bundleDir, resultSchema, actorDelivery) {
  ...
  return [
    ...
    '',
    '- Write every output, cache leaf, receipt event, and result under the current run bundle root. Do not mutate queue, index, ledger, beacon, or Gate state.',
    '- Verify every assigned required output against its listed direct contract before recording `work_done`.',
    `- The Phase Agent runs the existing same claimed attempt dry-submit after the actor returns: \`node DEEP_RESEARCH_HARNESS/cli/operate-work-unit.mjs dry-submit "${path.resolve(bundleDir)}" --work-id "${manifest.work_id}" --result "${abs.result_ref}"\`.`,
...
export function spawnPromptForWorkUnit(manifest, bundleDir = null, { actorDelivery } = {}) {
  ...
  return [
    actorInstruction,
    '',
    resolvedBundleDir ? `Current run bundle_dir: ${resolvedBundleDir}` : 'Read bundle_dir from the assigned _beacon.json before writing files.',
