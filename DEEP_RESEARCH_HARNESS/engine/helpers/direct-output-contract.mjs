...
function readBoundedSnapshot(bundleDir, target, contractId) {
  ...
  try {
    ...
    return failure(common, {
      ...
      expected: 'A readable current run bundle root.',
      observed: 'The current run bundle root could not be resolved.',
      rootClass: 'contract_integrity',
    });
  }
  ...
  try {
    ...
    if (targetStat.isSymbolicLink()) {
      return failure(common, {
        ...
        expected: 'A regular non-symlink target inside the current run bundle.',
    ...
    return failure(common, {
      ...
      expected: 'A readable regular target inside the current run bundle.',
  ...
  if (!isInside(targetReal, bundleReal)) {
    return failure(common, {
      ...
      expected: 'A target whose realpath remains inside the current run bundle.',
      observed: 'The target realpath escapes the current run bundle.',
      rootClass: 'contract_integrity',
    });
  }
