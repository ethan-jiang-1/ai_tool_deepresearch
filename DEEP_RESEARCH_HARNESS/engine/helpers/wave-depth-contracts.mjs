...
const FOCUS_OUTCOMES = new Set(['covered', 'partial', 'blocked']);
const FOCUS_COMMITMENT_STATES = new Set(['covered', 'limited']);
const FOCUS_BOUNDARY_KINDS = new Set(['external_action', 'user_decision', 'missing_contract']);
const FOCUS_ROOT_KEYS = new Set(['topic_uid', 'rerun_count', 'outcome', 'commitments']);
const FOCUS_COVERED_KEYS = new Set(['id', 'statement', 'state', 'submitted_work_unit_refs']);
const FOCUS_LIMITED_KEYS = new Set(['id', 'statement', 'state', 'limitation', 'boundary_kind']);
...
function focusCoverageIssue(issues, code, detail) {
  issues.push({ code, detail: `[${code}] FAIL: ${detail}` });
}

function submittedFactByRef(facts) {
  const byRef = new Map();
