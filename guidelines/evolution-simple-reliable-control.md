...
- 影响恢复的 accepted intent、canonical identity 与 progress SHALL 在对应 dependent work 开始前进入 sanctioned Source of Record；不能让 chat memory 或完成后的产物反向成为唯一意图证明。
- 新取得的数据 SHALL 在 acquisition boundary 进入 crash-safe durable path，再进入下游提取或综合；这不要求预填尚未产生的研究内容。
...
回答这两问时，必须实际应用本文的 direct authority、blocking burden、state ownership、short-circuit、same-check repair、compatibility retirement、testability 与真实证据 disciplines。两问是入口，不是把详细纪律删成口号。若无法短答，先缩 scope、删除重复 rule 或拆掉派生链。
