...
| **`hints[]` / `resolution_owner`（gate/phase 门禁面）** | phase §7 门禁反馈主面的责任归属载体：`hints[]` 标明责任主体 `resolution_owner` ∈ {`agent_action` / `engine_operation` / `user_decision` / `external_action` / `missing_contract`}；明确告知谁来处置门禁未通过 |
...
|---|---|---|---|---|
| **Gate / Phase 门禁面** | `hints[].resolution_owner` | `agent_action`, `engine_operation`, `user_decision`, `external_action`, `missing_contract` | `openspec/specs/engine/check-inspect-feedback/spec.md` | 责任主体划分（归 Agent、Engine 还是 User 处置，回答 Who） |
