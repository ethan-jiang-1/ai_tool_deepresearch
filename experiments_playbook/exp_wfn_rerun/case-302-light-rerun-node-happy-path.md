...
trace: dpt_disp_case-302_rerun_happy_*/_trace.jsonl
verdict: trace-jsonl
---
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'hitl2 gate — rerun decision'})})"
```
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'chain-rerun-to-rerun-node',passed:$OK,detail:'rerun → phase-rerun.md'})})" $OK
```
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'rerun-ready',passed:$OK,detail:'gate pass → seed-topics'})})" $OK
```
