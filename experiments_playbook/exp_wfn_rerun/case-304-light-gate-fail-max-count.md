...
trace: dpt_disp_case-304_maxcount_*/_trace.jsonl
verdict: trace-jsonl
---
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'rerun-ready-max-count',passed:$OK,detail:'rerun_count=3 → gate fail'})})" $OK
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'rerun-ready-routing',passed:$OK,detail:'inspect+routing verified'})})" $OK
```
