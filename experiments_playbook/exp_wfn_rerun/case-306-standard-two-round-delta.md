...
trace: dpt_disp_case-306_tworound_*/_trace.jsonl
verdict: filesystem
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'round1-delta',passed:$R1_OK,detail:'rerun_count=1 + direction hints'})})" $R1_OK
```
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'round2-delta',passed:$R2_OK,detail:'rerun_count=2 + direction updated'})})" $R2_OK
```
