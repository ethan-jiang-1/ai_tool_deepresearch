...
trace: dpt_disp_case-303_normal_*/_trace.jsonl
verdict: trace-jsonl
---
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-normal-path',passed:$OK,detail:'normal path → readiness'})})" $OK
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'chain-passed-unchanged',passed:$OK,detail:'passed → readiness'})})" $OK
```
