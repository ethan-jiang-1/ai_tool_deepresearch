...
trace: dpt_disp_case-301_dualexit_*/_logs/_logs/_trace.jsonl
verdict: trace-jsonl
---
...
7. 从 `_logs/_logs/_trace.jsonl` 裁决
8. Cleanup
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_logs/_logs/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'rerun — gate passes'})})"
```
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_logs/_logs/_trace.jsonl',{gate:'chain-rerun',passed:$OK,detail:'rerun → phase-rerun.md'})})" $OK
```
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_logs/_logs/_trace.jsonl',{gate:'chain-passed',passed:$OK,detail:'passed → readiness'})})" $OK
```
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_logs/_logs/_trace.jsonl',{gate:'chain-indeterminate',passed:$INDET_OK,detail:'indeterminate → invalid_input'})})"
```
