...
case: case-106-light-hitl1-topic-rewrite-vague
weight: light
...
bundle: dpt_disp_case-106_wff_rw_*
trace: dpt_disp_case-106_wff_rw_*/_trace.jsonl
verdict: trace-jsonl
---
...
**⚠️ 模拟声明：** 本 playbook 中 original topic 和 seed topics 的内容由 bash 脚本写入（模拟 Agent 读取 `phase-hitl1.md` §3a 后的输出）。它验证的是：Agent 按 MD 指令产出的格式能被 gate CLI 正确处理。它**不验证** Agent 的语义判断质量——那需要真实 Agent 执行（见 `exph_workflow-foundation/case-901-heavy-topic-rewrite-agent.md`，及其 AI 扮演真人对偶 `case-951`）。

# case-106-light-hitl1-topic-rewrite-vague
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments_env/shared/new-disposable-bundle.mjs wff_rw --case case-106 --force)
echo "Bundle: $B"
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl1-recorded',passed:$PASSED,detail:'topic rewrite: vague -> structured per phase-hitl1.md 3a'})})"
```

## Step 6: 结果解读

> 验证 phase-hitl1.md §3a topic rewrite：
>   模糊输入 → Agent 应做 topic rewrite → 产出 original topic + seed topics。
