...
trace: dpt_disp_case-211_agql_w0_happy_*/_logs/_logs/_trace.jsonl
verdict: trace-jsonl
req: AGQ-008
---
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_logs/_logs/_trace.jsonl',{gate:'wave0-complete',passed:$PASSED,detail:'happy path: seed_topics→wave0 queue-loop→sub-agent real search→backfill→gate pass'})})"
```
...
grep -c 'queue_' $B/_logs/_logs/_trace_agq_cli.jsonl
