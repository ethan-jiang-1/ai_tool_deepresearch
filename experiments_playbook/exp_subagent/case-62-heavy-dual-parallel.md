...
experiment: subagent
case: case-62-heavy-dual-parallel
weight: heavy
...
trace: dpt_disp_case-62_gs_medium/rb_trace.jsonl
verdict: trace-jsonl
---
...
1. Engine dispatch 2 slots: source_intake + source_diagnostic [MAIN/SHELL]
2. Parent 记录 2 个 spawn request [MAIN/SHELL]
3. Parent 并行启动 2 个 native sub-agent [MAIN→SUBAGENT]
4. Sub-agents 写 runtime-receipt.jsonl → 返回 strict JSON [SUBAGENT]
5. Parent 导入 receipts → 校验 → commit → collect → merge → audit [MAIN/SHELL]
6. Trace 验证: 2×spawn, 2×runtime_started, 2×result_ready, 2×received, 2×validated

## Phase 1: Prepare Bundle And Dispatch [MAIN/SHELL]

```bash
B=$(node experiments_env/shared/new-disposable-bundle.mjs gs_medium --case case-62 --force)
...
for (const slot of slots) console.log(`Slot task: dpt_disp_case-62_gs_medium/${slot.taskPath}`);
JS
...
- **Slot 0** (`source_intake`): project agent `dpt-source-intake`
  - Read `dpt_disp_case-62_gs_medium/_subagents/wave_01/slot_00/task.md`
  - Read `dpt_disp_case-62_gs_medium/_subagents/wave_01/slot_00/result.schema.json`
  - Write `dpt_disp_case-62_gs_medium/_subagents/wave_01/slot_00/runtime-receipt.jsonl`
  - Follow the concrete task in its `task.md`.
  - Return strict JSON.

- **Slot 1** (`source_diagnostic`): project agent `dpt-source-diagnostic`
  - Read `dpt_disp_case-62_gs_medium/_subagents/wave_01/slot_01/task.md`
  - Read `dpt_disp_case-62_gs_medium/_subagents/wave_01/slot_01/result.schema.json`
  - Write `dpt_disp_case-62_gs_medium/_subagents/wave_01/slot_01/runtime-receipt.jsonl`
  - Follow the concrete task in its `task.md`.
  - Return strict JSON.
...
- Slot 0 → `dpt_disp_case-62_gs_medium/relay-source-intake.json`
- Slot 1 → `dpt_disp_case-62_gs_medium/relay-source-diagnostic.json`
...
> 验证 2 个 sub-agent 并行：
>   source_intake + source_diagnostic 并行 spawn
