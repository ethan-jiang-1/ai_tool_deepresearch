...
trace: dpt_disp_case-63_gs_identity/rb_trace.jsonl
verdict: trace-jsonl
---
...
```bash
B=$(node experiments_env/shared/new-disposable-bundle.mjs gs_identity --case case-63 --force)
...
for (const slot of slots) console.log('Slot task: dpt_disp_case-63_gs_identity/' + slot.taskPath);
JS
...
- Slot 0 → `dpt_disp_case-63_gs_identity/relay-apple-research.json`
- Slot 1 → `dpt_disp_case-63_gs_identity/relay-toyota-research.json`
- Slot 2 → `dpt_disp_case-63_gs_identity/relay-nestle-research.json`
...
> 验证 3 个 sub-agent 身份独立性：
>   同一模板 dpt-source-intake，不同任务（Google Scholar/OpenAI/Anthropic）
>   → 3 个不同 agentId + 3 个不同 source URL。
>   trace 证明 uniqueness。
