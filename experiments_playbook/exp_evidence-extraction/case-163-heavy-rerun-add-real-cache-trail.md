...
After the initial `economic-impact` attempt submits, enqueue one normal supplementary `wave1_topic_deepening` demand for that same canonical Topic. Its generated `Authorized Source-Ref Lineage` must list the exact prior submitted `evidence_summary`. The supplementary actor may cite that exact path in `source_claims[].source_ref` without redeclaring or overwriting it, while all new cache/degraded refs remain current-attempt declarations. Wrong Topic/wave/kind/role or filesystem-only paths remain invalid.
