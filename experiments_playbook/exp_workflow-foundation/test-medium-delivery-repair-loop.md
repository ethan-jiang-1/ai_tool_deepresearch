...
case: medium-delivery-repair-loop
weight: light
...
bundle: dpt_disp_dlv_repair_*
trace: dpt_disp_dlv_repair_*/_trace.jsonl
verdict: trace-jsonl
---
...
# test-medium-delivery-repair-loop
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs dlv_repair --force)
echo "Bundle: $B"
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'attempt 1 — missing brief, expected fail'})})"
...
ADVICE=$(echo "$GATE_OUTPUT" | node experiments/shared/extract-field.mjs advice.0)
...
PASSED=$(echo "$GATE_OUTPUT" | node experiments/shared/extract-field.mjs check.passed)
echo "gate: hitl2-recorded | passed: $PASSED"
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'attempt 2 — brief created, expected pass'})})"
```
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'readiness-passed',passed:$PASSED,detail:'attempt 1 — missing synthesis, expected fail'})})"
...
PASSED=$(echo "$GATE_OUTPUT" | node experiments/shared/extract-field.mjs check.passed)
echo "gate: readiness-passed | passed: $PASSED"
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'readiness-passed',passed:$PASSED,detail:'attempt 2 — synthesis created, expected pass'})})"
```
