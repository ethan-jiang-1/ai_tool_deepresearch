...
case: complex-pre-research-review-surface
weight: light
...
bundle: dpt_disp_wff_review_*
trace: dpt_disp_wff_review_*/_trace.jsonl
verdict: trace-jsonl
---
...
# test-workflow-foundation-complex-review-surface
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs wff_review --force)
echo "Bundle: $B"
```
...
import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m => {
  m.recordCheck('$B/_trace.jsonl', { gate: 'hitl1-recorded', passed: $PASSED, detail: 'review surface: AI interpretation sample payload' });
});
"
```
