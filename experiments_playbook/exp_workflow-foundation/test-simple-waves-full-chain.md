...
bundle: dpt_disp_wf_chain_*
trace: dpt_disp_wf_chain_*/_trace.jsonl
verdict: trace-jsonl
---
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs wf_chain --force)
echo "Bundle: $B"
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'seed-topics-ready',passed:$PASSED,detail:'seed topics materialized — advancing to wave0'})})"
```
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'wave0-complete',passed:$PASSED,detail:'reference metadata collected — advancing to wave1'})})"
```
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'wave1-complete',passed:$PASSED,detail:'skeleton with placeholder marker — advancing to wave2'})})"
```
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'wave2-complete',passed:$PASSED,detail:'cross-topic synthesis with valid links — advancing to hitl2'})})"
```
