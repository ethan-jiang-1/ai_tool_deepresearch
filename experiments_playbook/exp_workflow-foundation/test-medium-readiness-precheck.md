...
case: medium-readiness-precheck
weight: light
...
bundle: dpt_disp_rd_pre_*
trace: dpt_disp_rd_pre_*/_trace.jsonl
verdict: trace-jsonl
---
...
# test-medium-readiness-precheck
...
7. 从 `_trace.jsonl` 裁决（预期 1 pass + 4 fail）
8. Cleanup
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs rd_pre --force)
echo "Bundle: $B"
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'readiness-passed',passed:$PASSED,detail:'happy path — all artifacts + all prior gates passed'})})"
```
...
PASSED=$(echo "$GATE_OUTPUT" | node experiments/shared/extract-field.mjs check.passed)
echo "gate: readiness-passed | passed: $PASSED"
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'readiness-passed',passed:$PASSED,detail:'missing synthesis — expected fail'})})"
...
PASSED=$(echo "$GATE_OUTPUT" | node experiments/shared/extract-field.mjs check.passed)
echo "gate: readiness-passed | passed: $PASSED"
INSPECT=$(echo "$GATE_OUTPUT" | node experiments/shared/extract-field.mjs inspect.0)
echo "inspect: $INSPECT"
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'readiness-passed',passed:$PASSED,detail:'partial gates — inspect must name missing gates'})})"
...
PASSED=$(echo "$GATE_OUTPUT" | node experiments/shared/extract-field.mjs check.passed)
echo "gate: readiness-passed | passed: $PASSED"
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'readiness-passed',passed:$PASSED,detail:'corrupt YAML — expected fail'})})"
...
PASSED=$(echo "$GATE_OUTPUT" | node experiments/shared/extract-field.mjs check.passed)
echo "gate: readiness-passed | passed: $PASSED"
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'readiness-passed',passed:$PASSED,detail:'corrupt JSONL — expected fail'})})"
```
