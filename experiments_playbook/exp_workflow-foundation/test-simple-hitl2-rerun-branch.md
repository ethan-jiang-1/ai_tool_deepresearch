...
case: simple-hitl2-rerun-branch
weight: light
...
bundle: dpt_disp_h2_rerun_*
trace: dpt_disp_h2_rerun_*/_trace.jsonl
verdict: trace-jsonl
---
...
# test-simple-hitl2-rerun-branch
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs h2_rerun --force)
echo "Bundle: $B"
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'rerun — gate passes but Agent should restart'})})"
```
...
## Step 4: Verdict from `_trace.jsonl`
