...
case: complex-wave-review-surface
weight: light
...
bundle: dpt_disp_w2_review_*
trace: dpt_disp_w2_review_*/_trace.jsonl
verdict: trace-jsonl
---
...
# test-complex-wave-review-surface
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs w2_review --force)
echo "Bundle: $B"
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'wave2-complete',passed:$PASSED,detail:'review surface: 3-topic cross-topic synthesis with 12 valid references'})})"
```
