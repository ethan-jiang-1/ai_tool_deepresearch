...
case: medium-pre-research-fault-tolerance
weight: light
...
bundle: dpt_disp_wff_fault_*
trace: dpt_disp_wff_fault_*/_trace.jsonl
verdict: trace-jsonl
---
...
# test-workflow-foundation-medium-fault-tolerance
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs wff_fault --force)
echo "Bundle: $B"
```
...
REPO_ROOT=$(pwd)
rm -rf $B
B=$(node experiments/shared/new-disposable-bundle.mjs wff_fault2 --force)
echo "Bundle: $B"
```
...
"
rm -rf dpt_disp_wff_fault_* 2>/dev/null
```
