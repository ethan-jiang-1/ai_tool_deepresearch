...
case: simple-hitl2-decision-recorded
weight: light
...
bundle: dpt_disp_h2_dec_*
trace: dpt_disp_h2_dec_*/_trace.jsonl
verdict: trace-jsonl
---
...
# test-simple-hitl2-decision-recorded
...
6. 从 `_trace.jsonl` 裁决（预期 1 pass + 3 fail）
7. Cleanup
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs h2_dec --force)
echo "Bundle: $B"
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'happy path — valid decision brief + profile + trace'})})"
```
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'missing decision brief — expected fail'})})"
```
...
echo "gate: hitl2-recorded | passed: $PASSED"
echo "inspect: $INSPECT"
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'empty user_decision — expected fail'})})"
```
...
echo "gate: hitl2-recorded | passed: $PASSED"
echo "inspect: $INSPECT"
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl2-recorded',passed:$PASSED,detail:'invalid enum — expected fail'})})"
```
