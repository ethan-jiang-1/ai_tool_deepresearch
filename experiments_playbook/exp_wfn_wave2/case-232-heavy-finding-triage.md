...
trace: dpt_disp_case-232_agql_w2_triage_*/rb_trace.jsonl
verdict: trace-jsonl
...
2. Phase Agent 分类 3 findings: resolution (search=false) / emergent (record_only) / legacy+search (exploit_search, has receipt) [MAIN]
3. 写三件套 → gate pass [MAIN/SHELL]
4. 验证: 无 orphan, 分类正确, W2F-003 有 receipt_ref

## Phase 1: 创建 post-wave1 bundle（含 intentionally unresolved question）

```bash
REPO_ROOT=$(pwd)
B=$(node experiments_env/shared/new-disposable-bundle.mjs agql_w2_triage --case case-232 --force)
echo "Bundle: $B"
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/rb_trace.jsonl',{gate:'wave2-complete',passed:$PASSED,detail:'finding triage: 1 resolution + 1 emergent + 1 exploit_search, no orphan'})})"
```
...
> 验证 finding taxonomy 分类正确：
>   W2F-001 (resolution): search_required=false, use_existing_evidence
