...
bundle: dpt_disp_agql_w2_repair_
trace: dpt_disp_agql_w2_repair_*/_trace.jsonl
verdict: trace-jsonl
req: RWG-003, RWG-010
---
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs agql_w2_repair --force)
echo "Bundle: $B"
...
PASSED1=$(echo "$GATE1" | node experiments/shared/extract-field.mjs check.passed)
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'wave2-complete',passed:$PASSED1,detail:'gate fail: ledger missing HITL2 Handoff section + backfill tokens unreplaced'})})"
```
...
PASSED2=$(echo "$GATE2" | node experiments/shared/extract-field.mjs check.passed)
echo "=== Gate#2 PASSED=$PASSED2 (expected: true) ==="
test "$PASSED2" = "true" && echo "PASS: gate passed after repair" || echo "FAIL"

node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'wave2-complete',passed:$PASSED2,detail:'gate pass: after repair (added HITL2 Handoff section + replaced backfill tokens)'})})"
```
