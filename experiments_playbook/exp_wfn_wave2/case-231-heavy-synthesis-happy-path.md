...
trace: dpt_disp_case-231_agql_w2_happy_*/_logs/_logs/_trace.jsonl
verdict: trace-jsonl
...
node -e "import('$REPO_ROOT/experiments_env/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_logs/_logs/_trace.jsonl',{gate:'wave2-complete',passed:$PASSED,detail:'happy path: post-wave1→wave2 queue-driven synthesis→三件套→backfill→gate pass'})})"
```
