...
bundle: dpt_disp_agql_w2_search_
trace: dpt_disp_agql_w2_search_*/_trace.jsonl
verdict: trace-jsonl
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs agql_w2_search --force)
echo "Bundle: $B"
...
```bash
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'wave2-complete',passed:$PASSED,detail:'sub-agent search: v1待搜索→spawn→receipt→v2写结果→00_shared→gate pass'})})" 2>/dev/null
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.verdict('$B/_trace.jsonl')})" 2>/dev/null
```
