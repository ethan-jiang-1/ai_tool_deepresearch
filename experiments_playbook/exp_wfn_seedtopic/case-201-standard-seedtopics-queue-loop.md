...
JS
node DEEP_RESEARCH_HARNESS/cli/enter-phase.mjs --bundle "$B" --node phases/phase-seed-topics.md > "$B/case-201-enter-seed-topics.md"
node DEEP_RESEARCH_HARNESS/cli/advance-status.mjs --bundle "$B" --to setup_ready > "$B/case-201-advance-setup.json"
```
...
JS
for TASK in "$STATE"/case-201/*.task.json; do node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs enqueue "$B" --task "$TASK"; done
```
...
for N in 1 2 3; do
  ...
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs claim "$B" --actor main-agent > "$CLAIM"
...
JS
  node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs complete "$B" --result "$RESULT"
done
node DEEP_RESEARCH_HARNESS/cli/operate-queue.mjs claim "$B" --actor main-agent > "$STATE/case-201/drained.json"
```
...
B=$(node DEEP_RESEARCH_HARNESS/host_tools/agent-experiment-state.mjs get-bundle --context {{RUN_CONTEXT_SH}} --role verdict)
STATE=$(printf '%s' {{PLAYBOOK_STATE_DIR_SH}})
GATE=$(node experiments_env/shared/run-gate-with-monitor.mjs --bundle "$B" --gate seed-topics-ready -- node DEEP_RESEARCH_HARNESS/cli/gates/check-gate-seed-topics-ready.mjs --bundle "$B" --current-node phases/phase-seed-topics.md)
