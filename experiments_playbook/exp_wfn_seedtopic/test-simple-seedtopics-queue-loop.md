...
case: simple-seedtopics-queue-loop
weight: light
...
bundle: dpt_disp_agql_seed_
trace: dpt_disp_agql_seed_*/_trace.jsonl
verdict: trace-jsonl
req: AGQ-010
---
...
# test-simple-seedtopics-queue-loop
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs agql_seed --force)
echo "Bundle: $B"
...
执行命令：
```bash
# Agent 为 3 个 topic 各创建 task card JSON，然后 enqueue：
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'seed-topics-ready',passed:$PASSED,detail:'queue-driven seed topics materialized — 3 files with {index}_{slug}.md naming, V12 fields, gate pass'})})"
```
