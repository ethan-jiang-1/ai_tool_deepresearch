...
```bash
B=$(node experiments/shared/new-disposable-bundle.mjs agq_medium --nodes=experiments/prototype-agentic-queue/nodes-agentic-queue --force)
...
const trace = createTrace('dpt_disp_agq_medium/_trace_agq_cli.jsonl', { consoleEcho: false });
...
```bash
rm -rf "$(node experiments/shared/new-disposable-bundle.mjs agq_medium)"
echo "Cleaned up."
```
