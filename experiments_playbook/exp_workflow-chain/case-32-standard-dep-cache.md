...
trace: dpt_disp_case-32_wc_medium/_logs/_trace.jsonl
verdict: trace-jsonl
---
...
> 9 个 check，验证 dependency-first + cache 行为：
>   [chain:status/plan_dep_first/order_dep_first] 依赖在前：policy→context→entry
>   [s2:repeat1_loaded] repeat-1 加载成功
>   [s2:shared_lib_read] shared-lib 首次 file_read=1（读盘）
>   [s3:repeat2_loaded] repeat-2 加载成功
>   [s3:no_extra_read] shared-lib 无新 file_read（缓存命中）
>   [s3:cache_hit] 有 cache_hit event
