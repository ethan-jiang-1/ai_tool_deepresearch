...
case: complex
weight: light
case_goal: "验证 FSM transition 级 halt 场景：undefined outcome、unknown node，以及 halt 后的 recovery。错误不污染 Engine，可恢复运行合法 FSM。"
runner: coding-agent
...
```
wave.entry →(passed)→ wave-audit →(passed)→ wave-final →(passed)→ null
```
...
// ── halt: unknown node ──
{
  const fsm = { name: 'bad', initial: 'real.md', states: { 'real.md': { on: { passed: null } } } };
  const result = resolveTransition(fsm, 'nonexistent.md', 'passed');
...
JS
node $B/run.mjs $B $B/exp/nodes > /dev/null 2>&1
```
