...
case: heavy-hitl1-topic-rewrite-agent
weight: heavy
...
bundle: dpt_disp_wff_rwa_*
trace: dpt_disp_wff_rwa_*/_trace.jsonl
verdict: trace-jsonl
---
...
# test-workflow-foundation-heavy-hitl1-topic-rewrite-agent
...
```bash
REPO_ROOT=$(pwd)
B=$(node experiments/shared/new-disposable-bundle.mjs wff_rwa --force)
echo "Bundle: $B"
```
...
node -e "import('$REPO_ROOT/experiments/shared/wff-playbook-utils.mjs').then(m=>{m.recordCheck('$B/_trace.jsonl',{gate:'hitl1-recorded',passed:$PASSED,detail:'real Agent rewrite per phase-hitl1.md 3a'})})"
```
